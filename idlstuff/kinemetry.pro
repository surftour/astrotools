...
;
;
;  This is a slightly modified (by TJ) kinemetry.pro.
;  No functional changes, all cosmetic.
;
;
;
;
...
ENDIF ELSE BEGIN
	...
	;nrad = 10 ; Large value: it will be truncated anyway
	pix = FINDGEN(nrad)
	rad = pix + (1.1^pix)
ENDELSE
...
; loop over radii
;
print, "loop over radii, nrad= ", nrad
FOR i=0, nrad-1 DO BEGIN
	...
	fname=fitsdir+"/"+istring+"/radial_bin_"+strcompress(string(i),/remove_all)+".eps"
	setup_plot_stuff, 'ps', filename=fname, colortable= 0, newxsize= 18.0, newysize= 30.0

        ;!P.MULTI=[0,1,4]
	!p.position= [0.15, 0.77, 0.52, 0.99]
	!p.ticklen= 0.03
        ;sauron_colormap
	loadct, 25, /silent
		...
		CONTOUR, moment>mn<mx, xbin, ybin, /FILL, /ISO, LEVELS=levels, $
		    /XSTYLE, /YSTYLE, /IRREGULAR, XTITLE='kpc/h', YTITLE='kpc/h', charsize= 1.2
        ENDIF ELSE BEGIN
		...
		CONTOUR, (moment-median(moment))>(-mx)<mx, xbin, ybin, /FILL, /ISO, LEVELS=levels, $
		    /XSTYLE, /YSTYLE, /IRREGULAR, XTITLE='kpc/h', YTITLE='kpc/h', /noerase, charsize= 1.2
	ENDELSE
	...
	!p.position= [0.10, 0.53, 0.97, 0.74]
        LOADCT, 4, /silent
        CONTOUR, ALOG(chi2_grid), pa_grid, q_grid, NLEVELS=15, XTITLE='PA (deg)', YTITLE='q', /FILL, charsize= 1.2, /noerase
	...
	!p.position= [0.10, 0.26, 0.97, 0.48]

	IF KEYWORD_SET(EVEN) THEN BEGIN
	  IF N_ELEMENTS(error) gt 1 THEN $
	        PLOTERROR, theta[w]*!RADEG, momEll[w], er_momEll, PSYM=-1, TITLE=rad[i]*scale, XTICKFORMAT='(A1)', YTITLE='V', /noerase, charsize=1.2 $
	  ELSE $
        	PLOT, theta[w]*!RADEG, momEll[w], PSYM=-1, TITLE=rad[i], XTICKFORMAT='(A1)', YTITLE='V', /noerase, charsize=1.2
          OPLOT, theta*!RADEG, coeff[0]+coeff[2]*COS(theta), PSYM=-4, COLOR=200

          PLOT, theta*!RADEG, SIGRANGE(momEll-coeff[0]), PSYM=-4, XTITLE='!7h!X!N', YTITLE='residuals', YRANGE=[-20,20], charsize= 1.2, /noerase
	...
	ENDIF ELSE BEGIN
	  ...
	  IF N_ELEMENTS(error) gt 1 THEN $
	        PLOTERROR, theta[w]*!RADEG, momEll[w], er_momEll, PSYM=-1, TITLE=rad[i]*scale, XTICKFORMAT='(A1)', YTITLE='V', charsize= 1.2, /noerase $
	  ELSE $
        	PLOT, theta[w]*!RADEG, momEll[w], PSYM=-1, TITLE=rad[i], XTICKFORMAT='(A1)', YTITLE='V', charsize= 1.2, /noerase
          OPLOT, theta*!RADEG, coeff[0]+b1*COS(theta), PSYM=-4, COLOR=200

	  !p.position= [0.10, 0.04, 0.97, 0.26]

          PLOT, theta*!RADEG, SIGRANGE(momEll-coeff[0]-b1*COS(theta)), PSYM=-4, XTITLE='!7h!X!N', YTITLE='residuals', YRANGE=[-20,20], charsize= 1.2, /noerase
