pro img_makepic, x, y, z, m, xlen, xz=xz, yz=yz, $
			hsml=hsml, $
...
;if keyword_set(rotate_phi) then if strmid(rotate_phi,0,2) eq "xy" then goto, done_with_rotation
;if keyword_set(rotate_theta) then if strmid(rotate_theta,0,2) eq "xy" then goto, done_with_rotation

;or strmid(rotate_theta,0,2) eq "xy" then goto, done_with_rotation
;if strmid(rotate_phi,0,2) eq "xy" or strmid(rotate_theta,0,2) eq "xy" then goto, done_with_rotation

;
; probably smart way to do the following, but this will
; work for now
;
;if keyword_set(rotate_phi) then begin
;  if strmid(rotate_phi,0,2) eq "xz" then begin
;	x= x
;	temp= y
;	y= z
;	z= temp
;	goto, done_with_rotation
;  endif
;endif
;
;if keyword_set(rotate_theta) then begin
;  if strmid(rotate_theta,0,2) eq "xz" then begin
;	x= x
;	temp= y
;	y= z
;	z= temp
...
;endif
;
;if strmid(rotate_phi,0,2) eq "yz" or strmid(rotate_theta,0,2) eq "yz" then begin
;	temp= x
;	x= y
;	y= temp
;
;	temp= y   ; which is now the original x
;	y= z
;	z= temp
;
;	goto, done_with_rotation
