...
;
; CALLING SEQUENCE:
;   parms = MGE_MPFIT(MYFUNCT, start_parms, FUNCTARGS=fcnargs, NFEV=nfev,
...
;  MPFIT, and then the model function must be declared MGE_MPFITan
...
;   This version of MPFIT includes a small but important modification 
;      to improve convergency with the MGE_FIT_SECTORS program. 
;      Michele Cappellari, Windhoek, 3 October 2008 
;
;  $Id: mpfit.pro,v 1.52 2008/05/04 03:16:02 craigm Exp $
;-
...
pro MGE_MPFIT_dummy
  ...
  FORWARD_FUNCTION MGE_MPFIT_fdjac2, MGE_MPFIT_enorm, MGE_MPFIT_lmpar, MGE_MPFIT_covar, $
    MGE_MPFIT, MGE_MPFIT_call

  COMMON MGE_MPFIT_error, error_code  ;; For error passing to user function
  COMMON MGE_MPFIT_config, mpconfig   ;; For internal error configrations
end

;; Reset profiling registers for another run.  By default, and when
;; uncommented, the profiling registers simply accumulate.

pro MGE_MPFIT_resetprof
  COMPILE_OPT IDL2, HIDDEN
  common MGE_MPFIT_profile, MGE_MPFIT_profile_vals

  MGE_MPFIT_profile_vals = { status: 1L, fdjac2: 0D, lmpar: 0D, MGE_MPFIT: 0D, $
...
pro MGE_MPFIT_setmachar, double=isdouble
  COMPILE_OPT IDL2, HIDDEN
  common MGE_MPFIT_profile, profvals
  if n_elements(profvals) EQ 0 then MGE_MPFIT_resetprof

  common MGE_MPFIT_machar, MGE_MPFIT_machar_vals
  ...
  MGE_MPFIT_machar_vals = {machep: dmachep, maxnum: dmaxnum, minnum: dminnum, $
...
;; derivatives or not.
function MGE_MPFIT_call, fcn, x, fjac, _EXTRA=extra

  COMPILE_OPT IDL2, HIDDEN
  on_error, 2
  common MGE_MPFIT_config, mpconfig

  if keyword_set(mpconfig.qanytied) then MGE_MPFIT_tie, x, mpconfig.ptied
...
function MGE_MPFIT_fdjac2, fcn, x, fvec, step, ulimited, ulimit, dside, $
                 iflag=iflag, epsfcn=epsfcn, autoderiv=autoderiv, $
                 FUNCTARGS=fcnargs, xall=xall, ifree=ifree, dstep=dstep

  COMPILE_OPT IDL2, HIDDEN
  common MGE_MPFIT_machar, machvals
  common MGE_MPFIT_profile, profvals
  common MGE_MPFIT_error, mperr
...
;  prof_start = systime(1)
  ...
  if NOT keyword_set(autoderiv) then begin
      ...
      fp = MGE_MPFIT_call(fcn, xall, fjac, _EXTRA=fcnargs)
      iflag = mperr
...
;      profvals.fdjac2 = profvals.fdjac2 + (systime(1) - prof_start)
  ...
  for j=0L, n-1 do begin
      ...
      mperr = 0
      fp = MGE_MPFIT_call(fcn, xp, _EXTRA=fcnargs)
      ...
      if abs(dside[ifree[j]]) LE 1 then begin
          ...
          mperr = 0
          fm = MGE_MPFIT_call(fcn, xp, _EXTRA=fcnargs)
...
pro MGE_MPFIT_qrfac, a, ipvt, rdiag, acnorm, pivot=pivot
          ...
          for k=j+1, n-1 do begin
              ...
              if keyword_set(pivot) AND rdiag[k] NE 0 then begin
                  ...
                  if 0.05D*temp*temp LE MACHEP0 then begin
                      rdiag[k] = MGE_MPFIT_enorm(a[j+1:*,lk])
                      wa[k] = rdiag[k]
                  endif
              endif
          endfor
...
;     burton s. garbow, kenneth e. hillstrom, jorge j. more
;
pro MGE_MPFIT_qrsolv, r, ipvt, diag, qtb, x, sdiag
...
;     burton s. garbow, kenneth e. hillstrom, jorge j. more
;
function MGE_MPFIT_lmpar, r, ipvt, diag, qtb, delta, x, sdiag, par=par
...
;  prof_start = systime(1)
  ...
  temp = sqrt(par)
  wa1 = temp * diag
  MGE_MPFIT_qrsolv, r, ipvt, wa1, qtb, x, sdiag
  wa2 = diag*x
  dxnorm = MGE_MPFIT_enorm(wa2)
  temp = fp
  fp = dxnorm - delta
...
pro MGE_MPFIT_defprint, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, $
...
pro MGE_MPFIT_defiter, fcn, x, iter, fnorm, FUNCTARGS=fcnargs, $
  ...
  if n_params() EQ 3 then begin
      fvec = MGE_MPFIT_call(fcn, x, _EXTRA=fcnargs)
      fnorm = MGE_MPFIT_enorm(fvec)^2
  endif
  ...
  if n_elements(iterprint0) EQ 0 then iterprint = 'MGE_MPFIT_DEFPRINT' $
...
pro MGE_MPFIT_parinfo, parinfo, tnames, tag, values, default=def, status=status, $
                   n_param=n
...
;
;     **********
function MGE_MPFIT_covar, rr, ipvt, tol=tol
...
;
;     **********
function MGE_MPFIT, fcn, xall, FUNCTARGS=fcnargs, SCALE_FCN=scalfcn, $
  ...
  if n_params() EQ 0 then begin
      message, "USAGE: PARMS = MGE_MPFIT('MYFUNCT', START_PARAMS, ... )", /info
      return, !values.d_nan
  endif
  ...
  if n_elements(iterproc) EQ 0 then iterproc = 'MGE_MPFIT_DEFITER'
  ...
  if strupcase(iterproc) EQ 'MGE_MPFIT_DEFITER' AND n_elements(iterargs) EQ 0 $
  ...
  if n_elements(xall) EQ 0 then begin
      MGE_MPFIT_parinfo, parinfo, tagnames, 'VALUE', xall, status=status
  ...
  ;; TIED parameters?
  MGE_MPFIT_parinfo, parinfo, tagnames, 'TIED', ptied, default='', n=npar
  ...
  ;; FIXED parameters ?
  MGE_MPFIT_parinfo, parinfo, tagnames, 'FIXED', pfixed, default=0, n=npar
  pfixed = pfixed EQ 1
  pfixed = pfixed OR (ptied NE '');; Tied parameters are also effectively fixed

  ;; Finite differencing step, absolute and relative, and sidedness of deriv.
  MGE_MPFIT_parinfo, parinfo, tagnames, 'STEP',     step, default=zero, n=npar
  MGE_MPFIT_parinfo, parinfo, tagnames, 'RELSTEP', dstep, default=zero, n=npar
  MGE_MPFIT_parinfo, parinfo, tagnames, 'MPSIDE',  dside, default=0,    n=npar

  ;; Maximum and minimum steps allowed to be taken in one iteration
  MGE_MPFIT_parinfo, parinfo, tagnames, 'MPMAXSTEP', maxstep, default=zero, n=npar
  MGE_MPFIT_parinfo, parinfo, tagnames, 'MPMINSTEP', minstep, default=zero, n=npar
  ...
  ;; LIMITED parameters ?
  MGE_MPFIT_parinfo, parinfo, tagnames, 'LIMITED', limited, status=st1
  MGE_MPFIT_parinfo, parinfo, tagnames, 'LIMITS',  limits,  status=st2
  ...
  if NOT isext then begin
      ...
      fvec = MGE_MPFIT_call(fcn, xnew, _EXTRA=fcnargs)
      iflag = mperr
  ...
  catch_msg = 'calling MGE_MPFIT_SETMACHAR'
  sz = size(fvec[0])
  isdouble = (sz[sz[0]+1] EQ 5)

  MGE_MPFIT_setmachar, double=isdouble
...
;  prof_start = systime(1)
  ...
  xnew[ifree] = x
  if qanytied then MGE_MPFIT_tie, xnew, ptied
  ...
  iflag = 2
  if NOT isext then begin
      catch_msg = 'calling MGE_MPFIT_FDJAC2'
      fjac = MGE_MPFIT_fdjac2(fcn, x, fvec, step, qulim, ulim, dside, $
  ...
  catch_msg = 'calling MGE_MPFIT_QRFAC'
  MGE_MPFIT_qrfac, fjac, ipvt, wa1, wa2, /pivot
  ...
  catch_msg = 'calculating LM parameter (MGE_MPFIT_LMPAR)'
  par = MGE_MPFIT_lmpar(fjac, ipvt, diag, qtf, delta, wa1, wa2, par=par)
...
;      if ct GT 0 then wa2[wh] = llim[wh]
      ...
      if uct GT 0 then wa2[whu] = ulim[whu] ; Michele Cappellari, Windhoek 3/OCT/2008
      if lct GT 0 then wa2[whl] = llim[whl] ; Michele Cappellari, Windhoek 3/OCT/2008
      
  endelse

  wa3 = diag * wa1
  pnorm = MGE_MPFIT_enorm(wa3)
  ...
  mperr = 0
  catch_msg = 'calling '+fcn
  wa4 = MGE_MPFIT_call(fcn, xnew, _EXTRA=fcnargs)
  iflag = mperr
  if iflag LT 0 then begin
  ...
  ;; taken
  temp1 = MGE_MPFIT_enorm(alpha*wa3)/fnorm
    ...
    AND n_elements(fjac) GT 0 AND n_elements(ipvt) GT 0 then begin
      ...
      if n GT 0 AND sz[0] GT 1 AND sz[1] GE n AND sz[2] GE n $
        AND n_elements(ipvt) GE n then begin
          ...
          if n EQ 1 then $
            cv = MGE_MPFIT_covar(reform([fjac[0,0]],1,1), ipvt[0]) $
          else $
            cv = MGE_MPFIT_covar(fjac[0:n-1,0:n-1], ipvt[0:n-1])
...
;  profvals.mpfit = profvals.mpfit + (systime(1) - prof_start)
  ...
  catch_msg = 'saving MGE_MPFIT state'
  ...
  if execute(cmd) NE 1 then $
    message, 'ERROR: could not save MGE_MPFIT state'
