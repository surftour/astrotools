...
;       V2.31 Added compilation options, MC, Leiden 20 May 2002
...
END
;----------------------------------------------------------------------------
FUNCTION multi_gauss, pars, img, sigmaPSF, normPSF, xpeak, ypeak, theta
;
COMPILE_OPT IDL2, HIDDEN
...
FOR j=0,ngauss-1 DO BEGIN
    FOR k=0,N_ELEMENTS(sigmaPSF)-1 DO BEGIN
        ...
        g = gauss2d_mge(s[1:2], xpeak, ypeak, qx, qy, theta)
...
END
;----------------------------------------------------------------------------
PRO MGE_print_contours, img, ang, xc, yc, sol, $
        FILE=file, MAGRANGE=magRange, SCALE=scale, BINNING=binning, $
...
s = SIZE(img)
model = multi_gauss(sol, img, sigmaPSF, normPSF, xc, yc, ang)
