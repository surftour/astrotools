...
;	  COVER=cover, PLOT=plot, VERBOSE=verbose]
;
;INPUTS:
...
;	           but if kinematic map is large (and has many pixels) it can be passed
;	           through this keyword. To use kinemetry for photometry it is also 
...
;            binned in elliptical sectors (64 in angle and of thicknes equal to 10% of 
;            the given radius). When IMG used center can be also fitted (currently only in
;            photometry, so if keyword EVEN is not set - which usually means one is analysing a
;            velocity map - center is not fitted! An estimate of the center is given 
;            through XC and YC keywords. ERROR should be a 2D array
...
;            the case, set SCALE to required number. In order to be compatible with 
;            previous versions of kinemetry XBIN, YBIN and MOMENT still have to be passed
;            but they can be dummmy 1D variables.
...
;	           a good estimate of PA and Q are passed to the program. 
...
;            radii: 
;		       - the best ellipse (overploted on kin.moment map). In IMG
;		         keyword is set, image is scaled to the size of the ellipse.
...
;    of the galaxy. X,Y,FLUX are 1D arrays. They are still required, but are
...
;    must be passed via PAQ keword. Outputs are for each ellips of semi-major
...
;              XELLIP=xellip, YELLIP=yellip, /verbose,  /plot, 
;
;
;REVISION HISTORY:
...
;          for ODD moments. Thanks to Maxime Bois for prompting. DK, Hatfield, 03.09.2008.
;   v3.2 - Passing center values when no minimization is attempted and PAQ values are 
;          kept constant. DK, Oxford, 31.10.2008.
;   v3.3 - Minor correction for PA output: now it is measued East of North (also in
;          ODD case). Thanks to Maxime Bois. DK. Oxford, 07.11.2008.
;#######################################################################
PRO sauron_colormap
;
...
PRO kinem_extract_sector, img, theta, a, da, q, pa, xc, yc, momell, $
	xell, yell, INTERP=interp, ERROR=error, ER_MOMELL=er_momell, _EXTRA=extra
;
...
ang = p[0]/!RADEG
theta = range(0.0,2.0*!pi, 64) 


IF keyword_set(img) THEN BEGIN
	IF r lt 40 THEN BEGIN 
	  IF keyword_set(EVEN) THEN KINEM_EXTRACT_SECTOR, img, theta, r, r*0.1, p[1], ang, p[2], p[3], momell, xell, yell,$
 				                          /interp, MISSING=12345678, ERROR=error, er_momell=er_momell ELSE $
                              KINEM_EXTRACT_SECTOR, img, theta, r, r*0.1, p[1], ang, x0, y0, momell, xell, yell,$
                                  /interp, MISSING=12345678, ERROR=error, er_momell=er_momell
...
;
coeff = KINEM_FIT_TRIG_SERIES(theta, momEll, momFit, NTERMS=nterms,$
...
;        MAIN PROGRAM
;----------------------------------------------------------------------
PRO kinemetry_phot, xbin, ybin, moment, rad, pa, q, cf, IMG=img, X0=x0, Y0=y0,$
	NTRM=NTRM, ERROR=error, SCALE=scale, NRAD=nrad, NAME=name, $
	PAQ=paq, NPA=npa, NQ=nq, RANGEQ=rangeq, RANGEPA=rangepa, ALL=all,$ 
  EVEN=even, VSYS=VSYS, VELCIRC=velCirc, VELKIN=velkin,  BMODEL=bmodel,$
	XC=xc, YC=yc, ER_CF=er_cf, ER_PA=er_pa, ER_q=er_q, ER_XC=er_xc, $
	ER_YC=er_yc, XELLIP=xellip, YELLIP=yellip, RING=ring, RADIUS=radius,$
	COVER=cover, PLOT=plot, VERBOSE=verbose, NOGRID=nogrid

compile_opt idl2

IF N_ELEMENTS(ntrm) EQ 0 THEN NTRM=6	    ; number of terms in F.expansion
IF N_ELEMENTS(cover) EQ 0 THEN cover = 0.75 ; required coverage of data

;
; make sure keywords are set correctly
;
IF KEYWORD_SET(even) THEN even=1 ELSE even = 0	          ; even moments;
...
IF keyword_set(IMG) and keyword_set(EVEN) THEN BEGIN
	parinfo = REPLICATE({step:1.0,limits:[0d,0d],limited:[1,1]}, 4)
...
FOR i=0, nrad-1 DO BEGIN
          ...
          FOR j=0,npa*nq-1 DO $
	            chi2_grid[j] = kinem_fitfunc_ellipse([pa_grid[j], q_grid[j], x0,y0],$
        	                   XBAR=xbar, YBAR=ybar, MOMENT=moment, NTERMS=4, RAD=rad[i], $
		                         EVEN=even, IMG=img, X0=x0, Y0=y0);ERROR=error, 
        ...
        IF keyword_set(IMG) THEN BEGIN
           ...
           ENDIF ELSE BEGIN ;keyword_set(EVEN)
                ; center fitting in odd case not implemented yet!!!!
 	              par = [pa_mpf, q_mpf]
                fa = {MOMENT:moment, XBAR:xbar, YBAR:ybar, NTERMS:4, RAD:rad[i], MPF:1, $
	                   ERROR:error, EVEN:even, IMG:img, X0:x0, Y0:y0}
                sol = MPFIT('kinem_fitfunc_ellipse', par, FUNCTARGS=fa, FTOL=1e-3, $
                     PARINFO=parinfo, NFEV=ncalls, BESTNORM=fmin, PERROR=err_sol, /QUIET)
                PA_min = sol[0]
                q_min = sol[1]
    ...
    ;
	  ...
	  IF keyword_set(IMG) and keyword_set(EVEN) THEN BEGIN
        tmp = kinem_fitfunc_ellipse([pa_min, q_min, X0s, Y0s], $
              ...
              ERROR=error, ER_MOMELL=er_momEll, ALL=all, EVEN=even, VSYS=vsys, IMG=img)
    ENDIF ELSE BEGIN
	...
	print, format='(1i3, 1a11, 1f10.3, 4f10.3, 3i4)', i, '-th radius: ', $
    ...
    ; the image.
    ;
    IF N_ELEMENTS(w) LT N_ELEMENTS(xEll)*cover THEN break
		  ...
		  ENDIF ELSE BEGIN;KEYWORD_SET(img)
	       mn = MIN(SIGRANGE(moment),MAX=mx)
			   levels = mn + (mx-mn) * FINDGEN(64)/(64-1)
		 	   CONTOUR, moment>mn<mx, xbin, ybin, /FILL, /ISO, LEVELS=10, $
	...
	IF KEYWORD_SET(EVEN) THEN BEGIN
	  ...
	  mnp=min(SIGRANGE(momEll-coeff[0]), max=mxp)
          PLOT, theta*!RADEG, SIGRANGE(momEll-coeff[0]), PSYM=-4, XTITLE='!7h!X!N', YTITLE='residuals', YRANGE=[mnp,mxp]
...
; PA correction
;
IF N_ELEMENTS(PAQ) LE 1 THEN BEGIN
	...
	; moments and not then keyword EVEN is set
	;
...
IF KEYWORD_SET(bmodel) THEN BEGIN
   ...
   ENDIF ELSE BEGIN 
	...
	KINEM_TRIGRID_IRREGULAR, xellip, yellip, vrec, xbin, ybin, velkin, MISSING=-12345
	;
