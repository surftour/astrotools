...
for rep = 0,n_pics-1 do begin
   ...
   if n_elements(bhid) gt 1 then begin
	mwcenter= fload_blackhole_xyz('xyz',center=[0,0,0],idtofollow=mw_bhid)
	print, "mw center: ", mwcenter

	m31center= fload_blackhole_xyz('xyz',center=[0,0,0],idtofollow=m31_bhid)
	print, "m31 center: ", m31center

	bh_mid_center= fltarr(3)
	bh_mid_center[0]= (mwcenter[0] - m31center[0])/2. + m31center[0]
	bh_mid_center[1]= (mwcenter[1] - m31center[1])/2. + m31center[1]
	bh_mid_center[2]= (mwcenter[2] - m31center[2])/2. + m31center[2]
   endif else begin
	bh_mid_center= fload_blackhole_xyz('xyz', center=[0,0,0],idtofollow=bhid)
   endelse
...
;;;;;;;;;;;;; start  X-Y- projection of density
   ...
   ;x= fload_allstars_xyz('x',center=bh_mid_center)
   ;y= fload_allstars_xyz('y',center=bh_mid_center)
   ;z= fload_allstars_xyz('z',center=bh_mid_center)
   x= fload_gas_xyz('x',center=bh_mid_center)
   y= fload_gas_xyz('y',center=bh_mid_center)
   z= fload_gas_xyz('z',center=bh_mid_center)
   N=   long(n_elements(x))
