...
;process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/vcs/vc3bvc3b", 30, $
;					linecolor= 50, xislog=xislog, /fitbeta
;process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/vcs/vc3bvc3b_no", 30, $
;					linecolor= 150, xislog=xislog

;process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/vc3vc3e_2", 107, $
;					linecolor= 50, xislog=xislog, /fitbeta
;process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/vc3vc3e_no", 107, $
;					linecolor= 150, xislog=xislog, /fitbeta


process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/gfs/vc3vc3u_k", 30, linecolor= 200, xislog=xislog
process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/gfs/vc3vc3v_k", 30, linecolor= 170, xislog=xislog
process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/gfs/vc3vc3w_k", 30, linecolor= 140, xislog=xislog
process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/gfs/vc3vc3x2_k", 30, linecolor= 110, xislog=xislog
process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/gfs/vc3vc3y_k", 30, linecolor= 80, xislog=xislog
process_and_plot_onexrayprof, bins, xmax, xmin, "/raid4/tcox/gfs/vc3vc3z_k", 30, linecolor= 50, xislog=xislog
...
pro plot_surface_brightness, frun, sendto, filename=filename, smoothlen=smoothlen

if not keyword_set(sendto) then sendto='ps'
if not keyword_set(frun) then begin
   print, "  "
   print, "PROBLEM: plot_surface_brightness"
   print, "  "
   return
endif
...
plot, [0], [0], psym=-3, $
	...
	process_and_plot_brightnessprofile, N, Time, x, y, z, m, age, zmets, $
			xmin, xmax, bins, linecolor= 150, /includeerr
	...
	process_and_plot_brightnessprofile, N, Time, x, y, z, m, age, zmets, $
                        xmin, xmax, bins, linecolor= 50, /includeerr
	...
	process_and_plot_brightnessprofile, N, Time, x, y, z, m, age, zmets, $
                        xmin, xmax, bins, linecolor= 100, /includeerr
...
pro process_and_plot_onebandprofile, x, y, z, lum_band, xmin, xmax, bins, $
	...
	if not keyword_set(x_is_devac) then r_avg= 10^(r_avg)
...
;
pro plot_surface_brightness_comparison, sendto, filename=filename, smoothlen=smoothlen, $
					snapnum=snapnum
...
;xtit="R (h!E-1!Nkpc)"
xtit="R (kpc)"
if makeitdevac eq 1 then xtit="[R (kpc)]!E1/4!N"
;ytit="U mag kpc!E-2!N"
;ytit="B mag arcsec!E-2!N"
ytit="K mag arcsec!E-2!N"
...
plot, [0], [0], psym=-3, $
  ...
  if do_orbits_and_orient eq 1 then begin
   ...
   if do_vc3_time eq 1 then begin
	...
	snapnum= 15
	initlc = 10
	do_one_galaxy_brightnessprofile, frun, snapnum, linecolor= initlc
	do_one_galaxy_brightnessprofile, frun, snapnum+1, linecolor= initlc+10
	do_one_galaxy_brightnessprofile, frun, snapnum+2, linecolor= initlc+20
	do_one_galaxy_brightnessprofile, frun, snapnum+3, linecolor= initlc+30
	do_one_galaxy_brightnessprofile, frun, snapnum+4, linecolor= initlc+40
	do_one_galaxy_brightnessprofile, frun, snapnum+5, linecolor= initlc+50
	do_one_galaxy_brightnessprofile, frun, snapnum+6, linecolor= initlc+60
	do_one_galaxy_brightnessprofile, frun, snapnum+7, linecolor= initlc+70
	do_one_galaxy_brightnessprofile, frun, snapnum+8, linecolor= initlc+80
	do_one_galaxy_brightnessprofile, frun, snapnum+9, linecolor= initlc+90
	do_one_galaxy_brightnessprofile, frun, snapnum+10, linecolor= initlc+100
	do_one_galaxy_brightnessprofile, frun, snapnum+11, linecolor= initlc+110
	do_one_galaxy_brightnessprofile, frun, snapnum+12, linecolor= initlc+120
	do_one_galaxy_brightnessprofile, frun, snapnum+13, linecolor= initlc+130
	do_one_galaxy_brightnessprofile, frun, snapnum+14, linecolor= initlc+140
	do_one_galaxy_brightnessprofile, frun, snapnum+15, linecolor= initlc+150
   endif
...
pro do_one_galaxy_brightnessprofile, frun, snapnum, linecolor=linecolor, msg=msg
...
ymin = 25.0
	...
	Time= fload_time(1)
        ...
        ; ---------------------
        Ndisk= fload_npart(2)
        x=[x,fload_disk_xyz('x')]
        y=[y,fload_disk_xyz('y')]
        z=[z,fload_disk_xyz('z')]
        m=[m,fload_disk_mass(1)]
        dage=fload_disk_age(1)
        age=[age,float(Time-dage)]
        zmets= [zmets,fload_disk_z(1)]
        ...
        process_and_plot_brightnessprofile, N, Time, x, y, z, m, age, zmets, $
                        xmin, xmax, bins, linecolor= linecolor, /x_is_devac   ;, /includeerr
