...
; -------------------------------------
	...
	;setup_plot_stuff, sendto, filename=filename, colortable= 4
        setup_plot_stuff, sendto, filename=filename, colortable= 4, newxsize=9, newysize=20
	...
	x0= 0.005
	xs= 0.99    ; assumes 1 panels
	x1= x0+xs

        y0= 0.1040    ; have bottom colorbar
	ys= 0.4455   ; assumes 2 panels
        y1= y0+ys
	y2= y0+ys+ys
	...
	tv, Pic2, x0, y0, xsize=xs, ysize=ys, /normal
	...
	;  first row
	; -----------
	overplot_axes, x0, y1, x1, y2, TimeLbl1, xlen, extramsg='!8n2low'

	;  second row
	; -------------
	overplot_axes, x0, y0, x1, y1, TimeLbl2, xlen, extramsg="!8n2toolow"
...
pro overplot_axes, xx0, yy0, xx1, yy1, thetimelbl, xlen, $
	...
	ymax=  xlen
        ...
        xyouts, xx0+0.06, yy1-0.04, thetimelbl, /normal, size= 1.33, charthick=3.0, color= 0

	if keyword_set(extramsg) then begin
		xyouts, xx0+0.06, yy1-0.08, extramsg, /normal, size= 1.33, charthick=3.0, color= 0
	endif
