...
pro make_movie_4, startid=startid, numpart=numpart, junk
...
;frun= '/raid6/gbesla/ToyModel/run/Phi30_2' & rotateby= 90.0 * (!PI / 180.)
...
; if no blackholes make sure to comment out the _xyz and set the center to 0
;	center[0]= 0 ;fload_blackhole_xyz('x')
;	center[1]= 0 ;fload_blackhole_xyz('y')
;	center[2]= 0 ;fload_blackhole_xyz('z')

        center= fload_center_stars(1)

	;skipit= 0
	skipit= 1
	if skipit ne 1 then begin
...
;	z_new= sin(rotateby)*x + cos(rotateby)*z
	...
	hsml= fload_gas_hsml(1)
        ...
        if keyword_set(numpart) and keyword_set(startid) then begin
            x = fload_1gal_gas_xyz('x', startid, numpart)
            y= fload_1gal_gas_xyz('y', startid, numpart)
            z = fload_1gal_gas_xyz('z', startid,numpart)
            m=fload_1gal_gas_mass(1,startid,numpart)
