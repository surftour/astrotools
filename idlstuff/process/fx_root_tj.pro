;--------------------------------------------------------------------
;
;  TJ's version that allows for parameters to be passes
;
;
;--------------------------------------------------------------------
...
function fx_root_tj, xi, func, double = double, itmax = itmax, $
                            stop = stop, tol = tol, $
			    PARINFO=pp
  ...
  while (it lt itmax and cond ne 1) do begin
    ...
    if stop eq 0 and abs(root - x[2]) le tol then begin
        cond = 1
            ...
            evalFunc = call_function( func, root, pp )
