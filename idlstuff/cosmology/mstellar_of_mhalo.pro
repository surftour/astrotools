...
;;   to convert from halo mass in (Msun/h) to stellar mass in Msun
;;
;; default to their early-type measurements
;;
function mstellar_of_mhalo, log_Mhalo_over_h, MEAN=MEAN, $
	...
	Mhalo = alog10(Mhalo_obs)
		M = INTERPOL(Mstar,Mhalo,log_Mhalo_over_h)
		
	if (keyword_set(ROBUST)) then $
		M = M * (log_Mhalo_over_h GE Mhalo[3]) + $
				(log_Mhalo_over_h + (Mstar[3]-Mhalo[3])) * (log_Mhalo_over_h LT Mhalo[3]) 
		;;
		;; applies a flat efficiency below ~10^11 Msun, chosen at the lowest mass 
		;;  where the efficiencies are well-determined
		;;
		
	if (keyword_set(MEAN)) then M = log_Mhalo_over_h + alog10((0.046/0.27)*0.17/0.7)
