function hx_atten_fac, log_NH
	forward_function attenuated_spectrum
	keV=2.418d17
	d_nu = 0.01
	nu = (2.0 + 0.01*findgen(801))*keV
...
pro compare_atten_fac
	forward_function fit_atten_fac, attenuated_spectrum
	
	NH = 16. + 0.1*findgen(101)
	f  = 0.0*NH
	for i=0,n_elements(NH)-1 do f[i] = hx_atten_fac(NH[i])
