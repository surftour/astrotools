...
double l_band_HRH(double log_l_bol, double nu);
double return_ratio_to_hard_xray(double nu);
double return_ratio_to_b_band(double nu);
double l_band_GTR(double log_L_bol, double nu);
double l_band_MAR(double log_l_bol, double nu);
double ratio_of_vandenberk_to_continuum(double nu);
...
{
	double nu, *nu_vec, log_L_bol, l_band, *l_band_vec;
	char *MODEL_SPECTRUM_SET;
	char *band_check;
	char *SLOAN_SET;
	int BAND_KEY=0;	int MODEL_SPECTRUM_KEY; int SLOAN_KEY=0; int N_nu=0; int i_nu;
		// sets if simply calling the bolometric correction for a prefab band : 
		//		BB = B-band, IR = 15 micron, SX = 0.5-2 keV, HX = 2-10 keV
	...
	// easier to just have the calling routine pass a numerical key 
	/*
	band_check = (char *)argv[0];
		printf(" bandid = %s \n",&band_check);
		char *BB="BB"; if (!strcmp(band_check,BB)) {BAND_KEY=1; nu=-1.0;}
		char *IR="IR"; if (!strcmp(band_check,IR)) {BAND_KEY=2; nu=-2.0;}
		char *SX="SX"; if (!strcmp(band_check,SX)) {BAND_KEY=3; nu=-3.0;}
		char *HX="HX"; if (!strcmp(band_check,HX)) {BAND_KEY=4; nu=-4.0;}
	if (BAND_KEY==0) nu = *(double *)argv[0];
	log_L_bol = *(double *)argv[1];
	// read in optional key to use alternate model spectrum
	MODEL_SPECTRUM_KEY=0;
  	if (argc > 2) {
		MODEL_SPECTRUM_SET = argv[2];
		char *HRH="HRH"; 			if (!strcmp(MODEL_SPECTRUM_SET,HRH)) 		{MODEL_SPECTRUM_KEY=0;}
		char *MARCONI="MARCONI"; 	if (!strcmp(MODEL_SPECTRUM_SET,MARCONI)) 	{MODEL_SPECTRUM_KEY=1;}
		char *RICHARDS="RICHARDS"; 	if (!strcmp(MODEL_SPECTRUM_SET,RICHARDS)) 	{MODEL_SPECTRUM_KEY=2;}
	// now check if the sloan flag is set
	if (argc > 3) {
		SLOAN_SET = (char *)argv[3];
		char *SLOAN="SDSS"; if (!strcmp(SLOAN_SET,SLOAN)) SLOAN_KEY=1;
	}
  	}
	*/
	N_nu = *(int *)argv[0];
		//l_band_vec = calloc(N_nu,sizeof(double));
	nu_vec = (double *)argv[1];

	log_L_bol = *(double *)argv[2];
	MODEL_SPECTRUM_KEY = *(int *)argv[3];
	SLOAN_KEY = *(int *)argv[4];

	l_band_vec = (double *)argv[5];

	for (i_nu=0;i_nu<N_nu;i_nu++) {
	nu = nu_vec[i_nu];
	if (MODEL_SPECTRUM_KEY==0) l_band = log10(l_band_HRH(log_L_bol,nu));
	if (MODEL_SPECTRUM_KEY==1) l_band = log10(l_band_MAR(log_L_bol,nu));
	if (MODEL_SPECTRUM_KEY==2) l_band = l_band_GTR(log_L_bol,nu);
	double log_nu;
		if (nu <= 0.) {
			if (nu==-1.0) log_nu = log10(2.998e8/(4.4e-7));
			if (nu==-2.0) log_nu = log10(2.998e8/(1.5e-5));
			if (nu==-3.0) log_nu = log10(1.0 * 2.418e17);
			if (nu==-4.0) log_nu = log10(5.0 * 2.418e17);
		} else {log_nu = log10(nu);}
	// check if inside range of Vanden Berk spectrum (note, this is not the 
	//    exact range over which that spectrum is compiled, since there is 
	//    serious instrumental cutoff at the extreme edges)
	if ((SLOAN_KEY==1)&&(log_nu>14.575)&&(log_nu<15.500)) 
		l_band += log10(ratio_of_vandenberk_to_continuum(nu));
...
// code to return the vanden Berk mean spectrum -- has been carefully gap-repaired 
//  and the low-nu continuum recalibrated following Haziminaglou et al. 2006 and 
//  Richards et al. 2006 to give the correct continuum slope where the geometric 
//  averaging routine was flattening things out -- then clipped at the edges for 
//  instrumental/IGM absorption cutoff at the extreme high-nu/low-nu ends, 
//  and renormalized to the boxcar continuum -- so here is the final ratio to 
//  continuum over the appropriate nu range, normalized s.t. total luminosity in 
//  this range is conserved -- can then be overlayed on any template spectrum
double ratio_of_vandenberk_to_continuum(double nu)
{
if (nu<= 0.) {
	if (nu==-1.0) return 1.0147898;
	// the other prefab bands are clearly outside the appopriate range for this overlay
	return 1.0;
}
double log_nu[926] = {
14.575, 14.576, 14.577, 14.578, 14.579, 14.580, 14.581, 14.582, 14.583, 14.584, 14.585,
...
double nuLnu[926] = {
0.915,  0.956,  0.960,  0.938,  0.969,  0.969,  0.999,  0.915,  0.944, 0.930,  0.928, 
...
if ((log_nu_obs>=log_nu[0])&&(log_nu_obs<=log_nu[924])) {
	int n0 = (int )((log_nu_obs-log_nu[0])/0.001);
...
// load the spectrum based on the Marconi et al. intrinsic SED estimate, 
//   slightly updated with the better l_uv-l_2kev bisector measurement of 
//   Steffen et al. 2006
double l_band_MAR(double log_l_bol, double nu)
{
	double x = log_l_bol - 12.;
	double lband = 0.;
	double P0,P1,P2,P3;
	if (nu==(0.))  return pow(10.,log_l_bol);
	if (nu < 0.) {
		if (nu==(-1.)) {P0=1.4121099; P1=4.9966372; P2=-0.011281406; P3=-0.32436235;}
		if (nu==(-2.)) {P0=7547.5983; P1=26706.569; P2=-0.011281406; P3=-0.32436235;}
		if (nu==(-3.)) {P0= 7.8402033; P1= 35.862231; P2= 0.23692144; P3=-0.036786135;}
		if (nu==(-4.)) {P0= 7.5808429; P1= 31.297873; P2= 0.27465670; P3=0.0036837670;}
		lband = P0*pow(10.,P3*x) + P1*pow(10.,P2*x);
		return pow(10.,log_l_bol)/lband;
	}
	// the rest is simple, only a few breaks in the relevant power laws
	// start w. normalization at 1micron (calculated from a grid of runs for different 
	//   l_2500, etc. to get l_1micron(l_bol)
	P0=2.2636845; P1=8.0098684; P2=-0.011281406; P3=-0.32436235;
	lband = P0*pow(10.,P3*x) + P1*pow(10.,P2*x);
	double lmicron = pow(10.,log_l_bol)/lband;
	double c_light = 2.998e8;
	double nu_micron = c_light/(1.0e-6);
	double nu_2500 = c_light/(2.5e-7);
	double nu_1300 = c_light/(1.3e-7);
	double nu_1200 = c_light/(1.2e-7);
	double nu_500  = c_light/(0.5e-7);
	double nu_keV  = 2.418e17;
		if (nu <= nu_micron) return lmicron * pow(nu/nu_micron,1.0+2.0);
	double l_1300 = lmicron * pow(nu_1300/nu_micron,1.0-0.44);
		if (nu <= nu_1300) return lmicron * pow(nu/nu_micron,1.0-0.44);
	double l_1200 = l_1300  * pow(nu_1200/nu_1300,1.0+0.0);
		if (nu <= nu_1200) return l_1300 * pow(nu/nu_1300,1.0+0.0);
	double l_500 = l_1200  * pow(nu_500/nu_1200,1.0-1.76);
		if (nu <= nu_500) return l_1200 * pow(nu/nu_1200,1.0-1.76);
	// now cross the alpha-ox region
	double l_keV = pow(10.,1.4716642 + 0.78808875*log10(l_500));
		// re-fitting the steffen et al. relation more conveniently for our use here
		//   (just changing the points chosen for interpolation and the units to 
		//     solar luminosities)
		if (nu <= nu_keV) return l_500 * pow(10.0, log10(l_keV/l_500) * 
											log10(nu/nu_500)/log10(nu_keV/nu_500));
	// above 1keV, load the x-ray template
	double l_2keV = l_keV / return_ratio_to_hard_xray(nu_keV);
	return l_2keV * return_ratio_to_hard_xray(nu);
}



// load the spectrum based on the all-quasar template of Richards et al. 2006
double l_band_GTR(double log_L_bol, double nu)
{
...
44.43, 44.49, 44.54, 44.59, 44.65, 44.70, 44.74, 44.79, 44.83, 44.87, 44.90, 44.93, 44.96,
