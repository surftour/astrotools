...
/* 
	...
	The results are simply printed to stdout, in three columns : 
		...
		2 : corresponding bolometric luminosity (given the median bolometric 
				corrections as a function of luminosity), in (log_{10}(L [erg/s]))

		3 : comoving number density per unit log_{10}(luminosity) : 
	...
	Example use : 
        compile       ::    > gcc qlf_calculator -o qlf_calc
...
//		phi_bol_grid = calloc(n_l_bol_pts,sizeof(double));
	...
	*/
	...
	if (VOCAL==1) {
	fprintf(stderr," Output : L_obs     L_bol     phi \n");
	...
	//for(i_lbol=0;i_lbol<n_l_bol_pts;i_lbol++) printf(" %f %f %e \n",l_band_grid[i_lbol],l_bol_grid[i_lbol],phi_bol_grid[i_lbol]);
...
double l_band_dispersion(double log_l_bol, double nu)
{
	...
	if ((nu >= nu15)&&(nu< nuBB)) {
		...
		return sf1 + (sf2-sf1) * (log10(nu/nu15)/log10(nuBB/nu15));
	}
	if ((nu >= nuBB)&&(nu< nuSX)) {
		s0 = 0.08; beta = -0.20; s1 = 0.065;
			sf1 = s0 * pow(10.,beta*x) + s1;
