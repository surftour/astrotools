...
pro produce_multiplot, junk, filename=filename, snaparr=snaparr
...
filename='sjb_panel_gsa.eps'
snaparr(*)= 10
xlenarr(*)= 15.0

frunarr= ['/raid4/tcox/ssft/iso1a', $
	  '/raid4/tcox/ssft/iso1b', $
	  '/raid4/tcox/ssft/iso2a', $
	  '/raid4/tcox/ssft/iso2b', $
	  '/raid4/tcox/ssft/iso3a', $
	  '/raid4/tcox/ssft/iso3b']
...
;--------------------------------------
	...
	;
        setup_plot_stuff, sendto, filename=filename, colortable= 4, newxsize=30, newysize=20
	...
	;do_panel, frun, snap1, xlen, x0, y1, xs, ys, $
	;	rotate_phi=rotate_phi, rotate_theta=rotate_theta, /colorbar
	do_panel, frunarr[0], snaparr[0], xlenarr[0], x0, y1, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta
	    xyouts, x0+xs-0.1, y1+ys-0.05, 'iso1a', /normal, size=1.7, charthick=3.0, color= 0
	do_panel, frunarr[1], snaparr[1], xlenarr[1], x1, y1, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta
	    xyouts, x1+xs-0.1, y1+ys-0.05, 'iso1b', /normal, size=1.7, charthick=3.0, color= 0
	do_panel, frunarr[2], snaparr[2], xlenarr[2], x2, y1, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta
	    xyouts, x2+xs-0.1, y1+ys-0.05, 'iso2a', /normal, size=1.7, charthick=3.0, color= 0
	do_panel, frunarr[3], snaparr[3], xlenarr[3], x0, y0, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta
	    xyouts, x0+xs-0.1, y0+ys-0.05, 'iso2b', /normal, size=1.7, charthick=3.0, color= 0
	do_panel, frunarr[4], snaparr[4], xlenarr[4], x1, y0, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta
	    xyouts, x1+xs-0.1, y0+ys-0.05, 'iso3a', /normal, size=1.7, charthick=3.0, color= 0
	do_panel, frunarr[5], snaparr[5], xlenarr[5], x2, y0, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta
	    xyouts, x2+xs-0.1, y0+ys-0.05, 'iso3b', /normal, size=1.7, charthick=3.0, color= 0
	...
	; -----------
		...
		xyouts, 0.07, 0.83, msg, /normal, size= 1.7, charthick=3.0, color= 0   ; 0=black, 1=white
        	xyouts, x0+0.07, y2-0.05, '!6Gyr', /normal, size= 1.5, charthick=3.0, color= 0
        endif
	...
	;snaplbl= '('+strcompress(string(snapnum),/remove_all)+')'
		...
		if keyword_set(drawcenters) then begin
			...
			;idx= where(cp_time le fload_time(1))
                        idx= where((cp_time le (fload_time(1)+0.001)) and (cp_time gt 0.7))
			cp_cen1_x= cp_cen1_x(idx)
			cp_cen1_y= cp_cen1_y(idx)

			oplot, cp_cen1_x, cp_cen1_y, psym=-3, linestyle=1, color= 150, thick=4.0
			...
			; calculate nuclear sep diff
			nidx= n_elements(cp_cen1_x)
			x_diff= cp_cen1_x - cp_cen2_x
			y_diff= cp_cen1_y - cp_cen2_y
			z_diff= cp_cen1_z - cp_cen2_z
			r_diff= sqrt(x_diff*x_diff + y_diff*y_diff + z_diff*z_diff)
			r_diff= r_diff(nidx-1)
			...
			xlenlbl= strcompress(string(r_diff),/remove_all)
			if r_diff ge 1.0 then digs= 1
			if r_diff ge 10.0 then digs= 2
        		xlenlbl = strmid(xlenlbl,0,3+digs)
        		xlenlbl= '!6R!Dsep!N= '+xlenlbl
        		xyouts, x1-0.12, y0+0.03, xlenlbl, /normal, size= 1.5, charthick=3.0, color= 0
...
pro overplot_axes, xx0, yy0, xx1, yy1, thetimelbl, xlen, $
	...
	ymax=  xlen
        ...
        xyouts, xx0+0.02, yy1-0.05, '!6'+thetimelbl, /normal, size= 1.5, charthick=3.0, color= 0
