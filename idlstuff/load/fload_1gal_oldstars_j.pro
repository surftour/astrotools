Function fload_1gal_oldstars_j, typeofam, startid=startid, numpart=numpart, center=center, vcom=vcom, total=total
...
;  3 - z component
    ...
    if npart(2)+npart(3) lt 1 then begin
	print, "No stars: halt!!!"
	return, [0]
    endif
    ...
    ; -------------------------------------------------------
    oldstarsids= fload_oldstars_id(1)
    if keyword_set(startid) then begin

        idx = where((oldstarsids ge long(startid)) and (oldstarsids lt long(startid)+long(numpart)))
