Function fload_1gal_newstars_j, typeofam, startid=startid, numpart=numpart, center=center, vcom=vcom, total=total
...
;  3 - z component
    ...
    ; -------------------------------------------------------
    newstarsids= fload_newstars_id(1)
    if keyword_set(startid) then begin

        idx = where((newstarsids ge long(startid)) and (newstarsids lt long(startid)+long(numpart)))
