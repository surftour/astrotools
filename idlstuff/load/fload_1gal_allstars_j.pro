Function fload_1gal_allstars_j, typeofam, startid=startid, numpart=numpart, center=center, vcom=vcom, total=total
...
;  3 - z component
    ...
    ; -------------------------------------------------------
    allstarsids= fload_allstars_ids(1)
    if keyword_set(startid) then begin

	idx = where((allstarsids ge long(startid)) and (allstarsids lt long(startid)+long(numpart)))
