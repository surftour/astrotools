...
c     SUBROUTINE XSPEXRAV(Ear,Ne,Param,Ifl,Photar)
      SUBROUTINE XSPEXRAV(Ear,Ne,Ifl,Photar,z_redshift)
 
      IMPLICIT NONE
      INTEGER Ne , Ifl
c     REAL Ear(0:Ne) , Param(*) , Photar(Ne)
      REAL*8 Ear(0:Ne) , Photar(Ne), z_redshift
      REAL Param(8)
...
c       From Marconi et al 2004
      ...
      IF ( firstcall ) THEN
c        CALL xwrite('Compton reflection from neutral medium.',5)
c        CALL xwrite('See help for details.',5)
c        CALL xwrite('If you use results of this model in a paper,',5)
c        CALL xwrite
c    &  ('please refer to Magdziarz & Zdziarski 1995 MNRAS, 273, 837'
c    &              ,5)
c        CALL xwrite('Questions: Andrzej Zdziarski, aaz@camk.edu.pl',5)
         firstcall = .FALSE.
      ENDIF
...
c ec.NE.0
         ...
         IF ( scale.NE.0. ) THEN
            ...
            DO 200 j = 1 , Jmax
c               print *,j,Jmax,x(1),xm,ab_,ab0,gamma,ec,Spref(j)
               Spref(j) = Spref(j)*ABS(scale)*normfac
 200        CONTINUE
         ELSE
            DO 220 j = 1 , Jmax
...
c     print *,gamma,ec,Spref(1),Sppow(1),Sptot(1)
      RETURN
      END
...
c        print *,j,jtransh,Ab_,Ab0,xnor
...
c     element in from x
      ...
      REAL fgabnd
      CHARACTER contxt*80
c     CHARACTER*4 fgsolr0

c     CHARACTER fgsolr*4
c     EXTERNAL fgsolr
...
c     firstflag = .TRUE.
c     nold = -1
c     print *,In,X(In),N,Ab_,Ab0,firstflag,iopac,nold

c input standard abundances 
c     IF ( (fgsolr().ne.fgsolr0).or.firstflag ) THEN
         firstflag = .FALSE.
         ab_in=999999.
         ab0in=999999.
c        fgsolr0=fgsolr()
c        abo( 1)=12+log10(fgabnd('H '))
c        abo( 2)=12+log10(fgabnd('He'))
c        abo( 3)=12+log10(fgabnd('C '))
c        abo( 4)=12+log10(fgabnd('N '))
c        abo( 5)=12+log10(fgabnd('O '))
c        abo( 6)=12+log10(fgabnd('Ne'))
c        abo( 7)=12+log10(fgabnd('Na'))
c        abo( 8)=12+log10(fgabnd('Mg'))
c        abo( 9)=12+log10(fgabnd('Al'))
c        abo(10)=12+log10(fgabnd('Si'))
c        abo(11)=12+log10(fgabnd('S '))
c        abo(12)=12+log10(fgabnd('Cl'))
c        abo(13)=12+log10(fgabnd('Ar'))
c        abo(14)=12+log10(fgabnd('Ca'))
c        abo(15)=12+log10(fgabnd('Cr'))
c        abo(16)=12+log10(fgabnd('Fe'))
c        abo(17)=12+log10(fgabnd('Ni'))
      abo( 1)=12+log10(1.00e+00)
      abo( 2)=12+log10(8.51e-02)
      abo( 3)=12+log10(3.31e-04)
      abo( 4)=12+log10(8.32e-05)
      abo( 5)=12+log10(6.76e-04)
      abo( 6)=12+log10(1.20e-04)
      abo( 7)=12+log10(2.14e-06)
      abo( 8)=12+log10(3.80e-05)
      abo( 9)=12+log10(2.95e-06)
      abo(10)=12+log10(3.55e-05)
      abo(11)=12+log10(2.14e-05)
      abo(12)=12+log10(3.16e-07)
      abo(13)=12+log10(2.51e-06)
      abo(14)=12+log10(2.29e-06)
      abo(15)=12+log10(4.68e-07)
      abo(16)=12+log10(3.16e-05)
      abo(17)=12+log10(1.78e-06)
         ab(1) = abo(1)
         ab(2) = abo(2)
...
c     print *,Xnor,SIGMABFA,In,iopac 
 999  CONTINUE
      IF ( ierr .NE. 0 ) THEN
c        CALL xwrite(contxt, 10)
         SIGMABFA = 0.
