...
#ifndef IR_OVERHEAD //prevent multiple definitions
#define IR_OVERHEAD
...
/* #define USE_FULL_NEIGHBOR_CALC NO */
/* #define USE_FULL_NEIGHBOR_CALC YES */
/* If YES (not equal 0), then use a direct nearest
   neighbor calculation for each point along the ray.
   Much slower for lots of rays, but more reliable */
...
#define MAX_DISTANCE_FROM0  1200.0		/* End integration along a ray when x,y,z > this value */
...
#define FRACTIONAL_STEPSIZE   0.5       /* Fraction of h_sml to step out when using brute-force integration */
#define TREE_MIN_SIZE	     0.05		/* minimum size of a tree cell (in GADGET units) */
...
#define NUMBER_OF_NEIGHBORS    64       /* Number of nearest neighbors to find calculating local values */
#define LINE_NMAX                        5000   /* Maximum number of points along a ray out of the galaxy
                                                                                 (here just for bookkeeping, can be arbitrarily large, 
                                                                                  but in practice never needs to be above a few hundred) */
...
#define SUPERNOVA_TEMPERATURE		3.0e8			// Temp. of SN, ~10^8 K
#define SUPERNOVA_FACTOR_BETA		0.1				// immediate supernova fraction (beta)
#define SUPERNOVA_EVAPORATION_FACTOR_A0 3000.0		// max. evaporation factor A0
#define STAR_FORMATION_TIMESCALE_IN_GYR 4.5             // max. star formation timescale t0*
 ...
 */
struct LOCALVALS_s{
	int Ngbs;			/* store the number of nearest neighbors */
...
typedef struct LOCALVALS_s LOCALVAL;
...
struct Ray_s{
  ...
  int DEST_INDEX;			/* index for the particle which is the ray destination */
  ...
  float Temp;				/* integrated mass-weighted temperature along the ray */
  float *r;			/* vector of radial distances along the ray (from BH to observer) */
  float thermalSZ;              /* thermal SZ = integral of n_e * T */
  float kineticSZ;		/* kinetic SZ = integral of n_e * v_radial */
  float xrayL;			/* xray Luminosity = integral of L_x_i */
  LOCALVAL *local;              /* vector of structures storing local parameters at each step */

};// Ray_struct;
typedef struct Ray_s Ray_struct;
...
struct CELL_STRUCT{
  ...
  LOCALVAL *dU;					/* holds the gradients of the local variables */
...
/* additional quantities for radiative transfer calc */ 
  ...
  float  *L_lambda_stars;	// L_lambda Spectrum from stars
  float  *L_lambda_BH;	        // L_lambda Spectrum from BHs
  float  *f_lambda_stars;	// L_lambda Spectrum from stars
  float  *f_lambda_BH;	        // L_lambda Spectrum from BHs
  float  L_effective;	//effective luminosity of source
  float  f_effective;	//effective luminosity of source
  float  R_effective;	//effective radius of source
  float  L_effective_stars;	//effective luminosity of source
  float  f_effective_stars;	//effective luminosity of source
  float  R_effective_stars;	//effective radius of source
  float  L_effective_BH;	//effective luminosity of source
  float  f_effective_BH;	//effective luminosity of source
  float  R_effective_BH;	//effective radius of source
};
...
struct particle_star
{
  ...
  double *L_lambda;	// L_lambda Spectrum
  double *L_lambda_extincted;	// L_lambda Extincted Spectrum
};
...
struct particle_bh 
{
  ...
  float  L_bol;		// bolometric luminosity
  double *L_lambda;	// L_lambda Spectrum
  double *L_lambda_extincted;	// L_lambda Extincted Spectrum
};
