...
*/
  float theta, phi; 
  float nh_prefactor, ne_prefactor, kSZ_prefactor, xray_prefactor;
  float meanwt, tempfactor;
  float *on; 
  float *oz;
  Ray_struct *ray;
  ...
  */
  if (argc != 8) {
    fprintf(stderr, "Expected 8 arguments (found %d)\n",argc); 
    exit(0); 
  }
...
*/
  ...
  for (i=0; i<All.N_gas; i++) {
    ...
    PG[i].z = P3d[i+1]->z; 
    PG[i].xrayL = P3d[i+1]->Mass * 1.0e+10 * P3d[i+1]->rho * sqrt(P3d[i+1]->u);   /* make mass m_solar */
  } 
  printf("\n");


  printf("PG[1].u=    %g     PG[1].temp=    %g    PG[1].xrayL=     %g\n",PG[1].u,PG[1].temp,PG[1].xrayL);
  printf("PG[100].u=  %g     PG[100].temp=  %g    PG[100].xrayL=   %g\n",PG[100].u,PG[100].temp,PG[100].xrayL);
  printf("PG[4751].u= %g     PG[4751].temp= %g    PG[4751].xrayL=  %g\n",PG[4751].u,PG[4751].temp,PG[4751].xrayL);
  fflush(stdout);
  ...
  meanwt= 0.6 * PROTONMASS;
  tempfactor= meanwt * (2./3.) * 1.0e+10 * 8.617e-8 / BOLTZMANN;   /* 8.617e-8 is boltzmann in keV */
  xray_prefactor = 1.2e-24 * All.UnitDensity_in_cgs / (meanwt * meanwt) * sqrt(tempfactor);
  printf("xray_prefactor= %g\n",xray_prefactor);
  /* note that there is still a m_solar in there, i.e., to get erg/s, we still need to multiply my m_solar */
...
/* ----------------------------------------------------------- */
  ...
  printf("opening: /home2/tcox/CMBStuff/HealPix_RingPixels/healpixangles.txt\n");
  if(!(fd=fopen("/home2/tcox/CMBStuff/HealPix_RingPixels/healpixangles.txt","r"))) {
    printf("Can't open file: /home2/tcox/CMBStuff/HealPix_RingPixels/healpixangles.txt\n");
    exit(0); 
  }
...
/* ----------------------------------------------------------- */
  ...
  for (ray_num=0; ray_num<NUMBER_OF_RAYS; ray_num++) { 
    ...
    ray[ray_num].xrayL *= xray_prefactor;
    ...
    if(ray_num < 10)
      {
	printf("ray_num= %d    xrayL= %g\n", ray_num, ray[ray_num].xrayL); fflush(stdout);
      }
...
/*
    ...
    *oz = ray[ray_num].kineticSZ;
    ...
    oz = OUT_xrayL + ray_num;
    *oz = ray[ray_num].xrayL;
*/
