...
#define TREE_FRAC_HSML        0.5   /* Fraction of h_sml for minimum size of tree cells */
...
//#define NUMBER_OF_RAYS       1000	/* Number of rays from each source (will be changed to nearest perfect square) */

#define USE_FULL_NEIGHBOR_CALC NO	/* If YES (not equal 0), then use a direct nearest neighbor calculation for 
										each point along the ray. Much slower for lots of rays, but more reliable */
#define NUMBER_OF_NEIGHBORS    32	/* Number of nearest neighbors to find calculating local values */
...
#define USE_CELL_GRADIENTS	YES		/* option to calculate the effects of a gradient in a given quantity across the cells -- 
										  technically more accurate, but if TREE_FRAC_HSML < 1 it shouldn't make a significant 
										  difference (in other words, mainly useful for speeding up the calculation). It can 
										  be important for estimation near the BH where gradients are large. Although I 
										  *think* the bugs are fixed, use this option at your own risk */
...
typedef struct Ray_s{
  ...
  /* these quantities are added (used only if using brute-force integration method) */
