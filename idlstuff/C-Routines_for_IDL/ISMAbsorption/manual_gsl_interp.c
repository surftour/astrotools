...
#define DISCARD_STATUS(s) if (s != 0) { printf("interpolation error: %d",s); }

gsl_interp *
gsl_interp_alloc (const gsl_interp_type * T, int size)
{
  gsl_interp * interp;

  if (size < T->min_size)
    {
      printf("insufficient number of points for interpolation type\n");
      exit(0);
    }
  ...
  if (interp == NULL)
    {
      printf("failed to allocate space for interp struct\n");
      exit(0);
    }
  ...
  if (interp->state == NULL)
    {
      free (interp);          
      printf("failed to allocate space for interp state\n");
      exit(0);
    };
...
int
gsl_interp_init (gsl_interp * interp, const double x_array[], const double y_array[], int size)
{
  if (size != interp->size)
    {
      printf("data must match size of interpolation object\n");
      exit(0);
    }
