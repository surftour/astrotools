...
int flag;       //interpolation flag
int A_flag;     //Absorption flag
...
//Applies extinction
double * ApplyExtinction(double L_lambda[], double lambda[], int N_lambda, double NH, double Z);
//void ApplyExtinction(double L_lambda_extincted[], double L_lambda[], double lambda[], int N_lambda, double NH);
//double Extinction(double wavelength, double NH);
double Extinction(double wavelength, double NH, double Z);

//Constructor, Destructor,
//and Initialization routines
void Initialize(void);
void Clear(void);
void FreeAbsorption(void);
void FreeWavelength(void);
double * LoadLambda(void);
double * LoadAbsorption(void);
double * LoadAbsorptionNoMetals(void);
double Cruddace1974(double x);
double Morrison1983(double x);
double Morrison1983_no_metals(double x);
double g_bf(double x, double Te, int n);

//Stores Extinction and lambda arrays
double *lambda;
double *pl;
double R_v;
double A_V;
...
printf("Loading Lambda\n"); fflush(stdout);
	lambda   = LoadLambda();
printf("lambda 1= %g  100= %g  200= %g  300= %g  400= %g  499= %g\n",lambda[1],lambda[100],lambda[200],lambda[300],lambda[400],lambda[499]);
	...
	/* standard metallicity */
	A_x = LoadAbsorption();
/* printf("STD: Ax 1= %g  100= %g  200= %g  300= %g  400= %g  499= %g\n",A_x[1],A_x[100],A_x[200],A_x[300],A_x[400],A_x[499]); fflush(stdout); */
	acc = gsl_interp_accel_alloc();
	lin = gsl_interp_alloc(gsl_interp_linear,N_lambda);
	status= gsl_interp_init(lin,lambda,A_x,N_lambda);
printf("gsl_interp_init, lin, status= %d\n",status);

	/* no metallicity */
	A_x_nm = LoadAbsorptionNoMetals();
/* printf("no Z: Ax_nm 1= %g  100= %g  200= %g  300= %g  400= %g  499= %g\n",A_x_nm[1],A_x_nm[100],A_x_nm[300],A_x_nm[400],A_x_nm[499]); */
...
printf("gsl_interp_init, lin_nm, status= %d\n",status);

/*
for(i=0;i<N_lambda;i++)
{
	printf("i= %d  lambda= %g  A_x = %g  A_x_nm= %g\n",i,lambda[i],A_x[i],A_x_nm[i]);
}
fflush(stdout);
*/
...
double Extinction(double wavelength, double NH, double Z)
{
	...
	}else if(l>lambda[N_lambda-1])
	{
		...
		//absorption
		A_z = gsl_interp_eval(lin,lambda,A_x,l,acc);
		A_nm = gsl_interp_eval(lin_nm,lambda,A_x_nm,l,acc_nm);
		A = (Z/0.02)*(-0.4*A_V*A_z) - (-0.4*A_V*A_nm)*((Z/0.02)-1.0);
	}
/*
printf("A_V= %g    l= %g  ",A_V,l);
printf("l[0]= %g    l[%d-1]= %g  ",lambda[0],N_lambda,lambda[N_lambda-1]);
printf("A_z= %g    A_nm= %g  ",A_z,A_nm);
printf("A= %g\n",A); fflush(stdout);
*/
	return A;
}
...
/*
printf("i= %d  x (1/pow(10,lambda))= %g    A= %g\n",i,x,A[i]);
*/
	}
	A_flag = 1;
	return A;
