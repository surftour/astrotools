...
double GetCoolingTime(double u_old, double rho, double *ne_guess)
{
  ...
  u = u_old;
	...
	//printf("Getting cooling rate from U u %e rho %e ne_guess %e\n",u,rho,*ne_guess);
  LambdaNet = CoolingRateFromU(u, rho, ne_guess);
	//printf("Got cooling rate from U\n");
...
double CoolingRateFromU(double u, double rho, double *ne_guess)
{
  ...
  //printf("Converting u to temp u %e rho %e ne %e\n",u,rho,*ne_guess);
  temp = convert_u_to_temp(u, rho, ne_guess);
#ifndef SFR_METALS
  //printf("Finding Cooling rate temp %e rho %e ne %e\n",log10(temp),rho,*ne_guess);
...
void InitCool(void)
{
  ...
  ReadIonizeParams("/n/home03/tcox/Tools/C-Routines_for_IDL/ComputeMultiphase/TREECOOL");
