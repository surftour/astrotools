...
static struct NODE 
{ 
  float  center[2],len;  /* center and sidelength of treecubes */
  float  xmin[2],xmax[2];
  int    count;          /* total # of particles in cell      */
  struct NODE *father,*suns[4];
...
static float searchmin[2],searchmax[2];
...
float ngb2d_treefind(float xyz[2], int desngb, float hguess,int **ngblistback, float **r2listback,
		     float hmax, int *ngbfound)
{
#define SECFACTOR  1.2
#define  PI               3.1415927
  void ngb2d_treesearch(struct NODE *this);
  ...
  if(hguess>0)
      ...
      while(th->count>200)
	{
      ...
      sr=th->len * pow((1.0/(PI)*SECFACTOR)*desngb/((float)(th->count)),1.0/2);
      if(sr>hmax)
	sr=hmax;
    }
	  ...
	  if(sr<hmax)
	    {
	      if(numngb>5)
		{
		  sr*=pow((2.1*(float)desngb)/numngb,1.0/2);
		}
	      else
		{
		  sr*=2.0;
...
/* packs the particles 0...Npart-1 in tree */
{
  ...
  for(j=1,len=xmax[0]-xmin[0];j<2;j++)
    if((xmax[j]-xmin[j])>len)
      len=xmax[j]-xmin[j];

  len*=1.01;

  printf("2D: Enclosing max has length= %g\n",len); fflush(stdout);
...
*/
      ...
      while(1)
	{
	  ...
	  for(j=0,subnode=0,fak=1;j<2;j++,fak<<=1)   /* which sector do we go into */
      ...
      if(th->count==2)  /* cell was occcupied with one particle */
	{
	  while(1)
	    {
	      ...
	      for(j=0,subp=0,fak=1;j<2;j++,fak<<=1)    /* which sector is the original particle */
	      ...
	      if(numnodes>=MaxNodes)
		{
	      ...
	      for(j=0,subi=0,fak=1;j<2;j++,fak<<=1)   /* which sector is the new particle */
		if(Pos[i]->xyz[j]>th->center[j])
		  subi+=fak;
      ...
      for(j=0,subi=0,fak=1;j<2;j++,fak<<=1)
	if(Pos[i]->xyz[j]>th->center[j])
	  subi+=fak;
