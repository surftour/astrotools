...
int project_smooth_and_total(int argc,void *argv[])
{
  int i,j,k,n,ii;
  float dummy[2],xy[2];
  float *r2list;
  int   *ngblist;
  float r,r2,h,h2,hinv2,wk,wk2,u,*hmaxfp;
  int   ngbfound;
  float *vv;
	  ...
	  *vv=0;
	  ...
	  for(k=0;k<ngbfound;k++)
	      ...
	      if(r<h)
		{
		  ...
		  wk2 =( Kernel[ii]    + (Kernel[ii+1]-Kernel[ii])*(u-KernelRad[ii])*KERNEL_TABLE);
		  /* *vv = *vv + Mass[ngblist[k]] * wk; */
		  *vv = *vv + Mass[ngblist[k]] * wk2;
		}      
	    }
	}
    } 
