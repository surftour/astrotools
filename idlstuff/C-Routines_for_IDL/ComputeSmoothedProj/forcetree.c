...
int force_treebuild(void)
{
   ...
   */
  for(tr=0, nfree=NumPart, numnodestotal=0; tr<6; tr++)
    {
      ...
      while(1)
	{
	  if(th >= NumPart)  /* we are dealing with an internal node */
	      ...
	      if(nfreep->len < 1.0e-3*epsilon)
		{
		  ...
		  Part[p].PosPred[0]+=(1.0e-2)*Part[p].PosPred[0];
		  //subnode= 0;
		}
  ...
  for(i=0, th=startnode; i<numnodes; i++, th++)
      ...
      for(j=7,nn=-1; j>=0; j--)  /* preparations for non-recursive walk */
	{
	  if(nodes[th].suns[j] >= 0)
	    {
	      if(nodes[th].suns[j] >= NumPart) /* write it only into suns that are internal nodes */
  ...
  for(tr=0; tr<6; tr++)
    {
      if(numnodestree[tr])
	{
	  epsilon= dmax(All.SofteningTable[tr], All.SofteningTable[Part[target].Type]);

	  if(All.TypeOfOpeningCriterion==0 || Part[target].OldAcc==0)
...
void force_treeevaluate_single_BH(int tree, int targetpart, double epsilon)  
{
  ...
  tppos   = &Part[targetpart].PosPred[0];
  tpaccel = &Part[targetpart].Accel[0];
  tppot   = &Part[targetpart].Potential;
...
void force_treeevaluate_single(int tree, int targetpart, double epsilon) 
{
  ...
  oac= Part[targetpart].OldAcc * All.ErrTolForceAcc;
...
void force_treeevaluate_direct(int target, double one_over_s_of_a) 
{
  ...
  for(i=0; i<NumPart; i++)
    {
      epsilon= dmax(All.SofteningTable[Part[i].Type], All.SofteningTable[Part[target].Type]);
      ...
      dx= Part[i].Pos[0] + Part[i].Vel[0]*dt - Part[target].Pos[0];
      dy= Part[i].Pos[1] + Part[i].Vel[1]*dt - Part[target].Pos[1];
      dz= Part[i].Pos[2] + Part[i].Vel[2]*dt - Part[target].Pos[2];
      ...
      if(u>=1)
	{
	  ...
	  fac=Part[i].Mass*r_inv*r_inv*r_inv;
          ...
          Part[target].Accel[0]+= dx*fac;
	  Part[target].Accel[1]+= dy*fac;
	  Part[target].Accel[2]+= dz*fac;
	}
      else
	{
	  ii = (int)(u*KERN_LEN); ff=(u-knlrad[ii])*KERN_LEN;
      ...
      if(u>1.0e-4)
	{
	  ...
	  Part[target].Accel[0]+=  Part[i].Mass*fcorr[0];
	  Part[target].Accel[1]+=  Part[i].Mass*fcorr[1];
	  Part[target].Accel[2]+=  Part[i].Mass*fcorr[2];
	}
#endif
    }
}
...
#define  PI               3.1415927
  ...
  if(hguess>0)
      ...
      while(nodes[th].count>200)
	{
	  ...
	  if(nodes[th].suns[subnode]>=NumPart)
	    {
...
void dump_particles(void)
{
  ...
  sprintf(buffer, "particles.dat");
  fd= fopen(buffer, "w");
  fwrite(&NumPart, 1, sizeof(int), fd);

  for(i=1; i<=NumPart; i++)
    fwrite(&P[i].PosPred[0], 3, sizeof(float), fd);

  for(i=1; i<=NumPart; i++)
    fwrite(&P[i].Pos[0], 3, sizeof(float), fd);

  for(i=1; i<=NumPart; i++)
    fwrite(&P[i].VelPred[0], 3, sizeof(float), fd);

  for(i=1; i<=NumPart; i++)
    fwrite(&P[i].Vel[0], 3, sizeof(float), fd);

  for(i=1; i<=NumPart; i++)
    fwrite(&P[i].CurrentTime, 1, sizeof(float), fd);

  for(i=1; i<=NumPart; i++)
    fwrite(&P[i].MaxPredTime, 1, sizeof(float), fd);
  ...
  fwrite(&s_a_inverse, 1, sizeof(double), fd);
