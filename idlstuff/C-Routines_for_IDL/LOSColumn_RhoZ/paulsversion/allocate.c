...
{
  int g; 
  int n_sources = (All.N_star + All.N_bh); 

  for (g=0; g<All.N_gas; g++) {
    if((!(PG[g].PTYPE = (int*) malloc(n_sources*sizeof(int)))) || (!(PG[g].PID = (int*) malloc(n_sources*sizeof(int)))) || (!(PG[g].NH = (float*) malloc(n_sources*sizeof(float)))) || (!(PG[g].D = (float*) malloc(n_sources*sizeof(float))))) 
    {
      fprintf(stderr,"failed to allocate memory.\n");
      exit(0);
    }
