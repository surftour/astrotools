...
int getnh(int argc, char *argv[0]) 
{
  ...
  if (argc != 6) {
    fprintf(stderr, "Expect arguments: snapshot theta phi [ID nh]\n"); 
    exit(0); 
  }
...
if (1) { 
  ...
  fprintf(stderr, "NUMBER_OF_RAYS = %d, N_rays = %d\n", NUMBER_OF_RAYS, N_rays); 
