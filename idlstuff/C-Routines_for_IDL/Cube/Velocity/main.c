...
int velocity(int argc,void *argv[])
{
  int i,j,k,n,ii;
  float dummy[3],xyz[3];
  float *r2list;
  ...
  for(i=0;i<Ngrid;i++)
    {
	  ...
	  */
	  ...
	  for(k=0;k<ngbfound;k++)
	      ...
	      if(r<h)
		{
		  ...
		  wk =hinv3*( Kernel[ii]    + (Kernel[ii+1]-Kernel[ii])*(u-KernelRad[ii])*KERNEL_TABLE);
                  ...
                  Vel_x += P3d[ngblist[k]]->Vel[0] * Mass[ngblist[k]] * wk;
                  Vel_y += P3d[ngblist[k]]->Vel[1] * Mass[ngblist[k]] * wk;
                  Vel_z += P3d[ngblist[k]]->Vel[2] * Mass[ngblist[k]] * wk;
	  ...
	  vv = Value + i*NumVelocity + 1;  *vv = Vel_y;
	  vv = Value + i*NumVelocity + 2;  *vv = Vel_z;
    } 
