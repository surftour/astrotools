...
int mass(int argc,void *argv[])
{
  ...
  ngb3d_treeallocate(Ngrid, 10*Ngrid);

  set_particle_pointer();

  ngb3d_treebuild((float **)&P3d[1], Ngrid, 0,dummy,dummy);
...
*/
	  ...
	  */
	  ...
	  h= 0.05;
	  h2=ngb3d_treefind( xyz, DesNgb ,1.04*h, &ngblist, &r2list, hsml, &ngbfound); 
...
/*
          if(!(i%10000))
	    {
              printf("%d..  h= %g  ngbfound= %d  wk_total= %g\n",i,h,ngbfound,wk_total); fflush(stdout);
	    }
*/
