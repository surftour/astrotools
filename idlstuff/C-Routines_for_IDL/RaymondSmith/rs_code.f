...
C  Note that the default abundances are Allen 1973 cosmic values. 
	...
	open(unit=7,file='answers.dat')
      ...
      print *, 'HELLO.  THIS PROGRAM COMPUTES EQUILIBRIUM OPTICALLY THIN
     1 X-RAY AND EUV EMISSION SPECTRA.  ENTER NBIN, BINMIN, BINSYZ.'
      READ *,NBIN,BINMIN,BINSYZ
      print *,' WAVELENGTH BINS MAYBE? NBLN,BLNMIN,BLNSYZ'
      READ *,NBLN,BLNMIN,BLNSYZ
   ...
   20 continue
      ...
      print *, 'DO YOU WISH TO MODIFY ABUNDANCES?'
      READ (5,1) IAB
    1 FORMAT (A1)
      IF (IAB .NE. 'Y' .AND. IAB .NE. 'y') GO TO 30
   25 CONTINUE
      print *, 'WANT CORONAL(C), PHOTOSPHERIC(P), OR 1 by 1(1)?'
      READ (5,1) IABSEL
      IF (IABSEL.EQ.'C'.OR.IABSEL.EQ.'c') go to 251
      IF (IABSEL.EQ.'P'.OR.IABSEL.EQ.'p') go to 253      
      print *, 'NO, ABUND?'
      READ (5,*) NO,ABUND
C
      ABUNJ(NO) = ABUND
C
      ...
      READ (5,1) IAB
      IF (IAB .EQ. 'Y' .OR. IAB .EQ. 'y') GO TO 25
      GO TO 30
 251  CONTINUE
      DO 252 I=1,12
 252  ABUNJ(I)= ABCOR(I)
...
c  85   continue
   ...
   70 CONTINUE
      print *, ' ANOTHER SET OF PARAMETERS?'
      READ (5,1) IREP
      IF (IREP .EQ. 'Y' .OR. IREP .EQ. 'y') GO TO 20
      stop
      end
