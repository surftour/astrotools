      program tabpower
c
c we will tabulate the power emitted in a given x-ray band
c per cubic centimeter, per neutral H atom, as a function
c of:
c
c     electron number density
c     metallicity
c     redshift (just for shift of bandpass) 
c     temperature
c 
c (all of these are in log bins except for redshift)
c
c.............................................................
c
      parameter (nzredmax=20,nrmetmax=20,
     >ntempdegkmax=40,nedenmax=20)
      common/eparms/ebot,etop,zred,rmet,tempdegk,eden
      real*8 totpow,nh
      common/binlims/nzred,zredmin,zredmax,dzred,
     >nrmet,alrmetmin,alrmetmax,aldrmet,
     >ntempdegk,altempdegkmin,altempdegkmax,aldtempdegk,
     >neden,aledenmin,aledenmax,aldeden
      common/power/powergrid(nzredmax,nrmetmax,ntempdegkmax,nedenmax)
c
c
...
c
      nzred=nzredmax
c
      dzred=(zredmax-zredmin)/float(nzred-1)
c
c the metallicity in units of solar: rmet
c
      rmetmin=0.0001
      rmetmax=1.
c
      alrmetmin=alog10(rmetmin)
      alrmetmax=alog10(rmetmax)
c
      nrmet=nrmetmax
c
      aldrmet=(alrmetmax-alrmetmin)/float(nrmet-1)
c
c temperature in degrees K: tempdegk
c
      tempdegkmin=1.e4
      tempdegkmax=1.e9
c
      altempdegkmin=alog10(tempdegkmin)
      altempdegkmax=alog10(tempdegkmax)
c
      ntempdegk=ntempdegkmax
c
      aldtempdegk=(altempdegkmax-altempdegkmin)/float(ntempdegk-1)
c
c the number of electrons per cm^-3?: eden
c
      edenmin=1.e-9
      edenmax=1.
c
      aledenmin=alog10(edenmin)
      aledenmax=alog10(edenmax)
c
      neden=nedenmax
c
      aldeden=(aledenmax-aledenmin)/float(neden-1)
c
c
      ntot=nzred*nrmet*ntempdegk*neden
c
c...........................................................
c
      print *,'starting...'
c
      do izred=1,nzred,1
c
         zred=zredmin+float(izred-1)*dzred
c
      do irmet=1,nrmet,1
c
         rmet=10.**(alrmetmin+float(irmet-1)*aldrmet)
c
      do itempdegk=1,ntempdegk,1
c
c
         tempdegk=10.**(altempdegkmin+float(itempdegk-1)*aldtempdegk)
 
      do ieden=1,neden,1
c
c
         eden=10.**(aledenmin+float(ieden-1)*aldeden)
c
      call xspct(totpow)
c
      powergrid(izred,irmet,itempdegk,ieden)=totpow
c
      ico=ico+1
c
      itot=itot+1
c
      if (ico.gt.nbip) then 
         print *,'on ',itot,' of ',ntot,' grid cells'
c
         print *,izred,irmet,itempdegk,ieden,' z,met,t,eden,power=',
     >zred,rmet,tempdegk,eden,
     >powergrid(izred,irmet,itempdegk,ieden)
c
         ico=0
c
         nbip=int(ran3(iseed)*100.)+10
c
         endif
c
      enddo
...
c
     ...
     >neden,aledenmin,aledenmax,aldeden
      ...
      real zredbin(nzredmax)
      real rmetbin(nrmetmax)
      real tempdegkbin(ntempdegkmax)
      real edenbin(nedenmax)
c     
      print *,'outfile?'
      read (*,'(a)') outfile
c
      open(12,file=outfile,status='new',form='unformatted')
c
      write(12) ebot,etop
c
      write(12) nzred,zredmin,zredmax,dzred,
     >nrmet,alrmetmin,alrmetmax,aldrmet,
     >ntempdegk,altempdegkmin,altempdegkmax,aldtempdegk,
     >neden,aledenmin,aledenmax,aldeden
c
      do izred=1,nzred,1
         zredbin(izred)=zredmin+float(izred-1)*dzred
         enddo
c
         write(12) (zredbin(i),i=1,nzred,1)
c
      do irmet=1,nrmet,1
         rmetbin(irmet)=10.**(alrmetmin+float(irmet-1)*aldrmet)
         enddo
c
         write(12) (rmetbin(i),i=1,nrmet,1)
c
      do itempdegk=1,ntempdegk,1
         tempdegkbin(itempdegk)=
     >10.**(altempdegkmin+float(itempdegk-1)*aldtempdegk)
         enddo
c
         write(12) (tempdegkbin(i),i=1,ntempdegk,1)
c
      do ieden=1,neden,1
         edenbin(ieden)=10.**(aledenmin+float(ieden-1)*aldeden)
         enddo
c
         write(12) (edenbin(i),i=1,neden,1)
c
      do izred=1,nzred,1
      do irmet=1,nrmet,1
      do itempdegk=1,ntempdegk,1
c
         write(12)(powergrid(izred,irmet,itempdegk,ieden),ieden=
     >1,neden,1)
c
         enddo
         enddo
...
cc      character*1 IREP,IAB, IABSEL
	...
	COMMON/RESULT/CONCE(30),GNDREC(30),POWER(220),RHY,HENEUT,HEPLUS,
     1  DNE,PCOOL,POU,POT,RE,TU,PM(4)
      ...
      real*8 totpow
      common/secondabun/abunj2(12)
      DATA ABUNJ2/10.93,8.52,7.96,8.82,7.92,7.42,7.52,7.2,6.9,6.3,
     ...
     3  7.58,7.55,7.21,6.56,6.36,7.67,6.25/  
      ...
      save igodone
c
      if (igodone.eq.0) then
c
	open(unit=2,file='atomic.dat')
	open(unit=3,file='file2.dat')
c
        endif
c...............................
c
...
cc      WRITE (1,50) T,BINMIN,BINSYZ,ABUNJ
   50 FORMAT (' TEMPERATURE=',E10.2,' BINMIN,
     1  BINSYZ=',2F7.1,' ABUNJ',12F6.2)
cc      WRITE (1,60) (BIN(K),K=1,NBIN)
   60 FORMAT (10E12.3)
...
c	WRITE(7,151) T,BINMIN,BINSYZ
  151 FORMAT (1H1,5H TEMP,F10.0,10H    BINMIN,F7.2,10H    BINSYZ,F7.2,/,
	...
	HNUU = HNU + BINSYZ
c	WRITE(7,155) IB,HNU,RECEV(IB),BRMEV(IB),TUFEV(IB),BIN(IB),JB,HNUU,
c     1  RECEV(JB),BRMEV(JB),TUFEV(JB),BIN(JB)
...
c  300 WRITE(7,31) N,ABUND,T,DENE
   31 FORMAT ('0LINES FOR N=',I2,'     ABUND=',F5.2,'     T=',E9.3,
     1 '     DENE=',E9.3)
c      IF (N .GE. 6) WRITE(7,32) N,PM
c      WRITE(7,38) (K,CONCE(K),K=1,NN)
   32 FORMAT (' CONCE FOR N=',I3,' METASTABLES ',4E10.3)
   38 FORMAT (10(I3,E10.2),/,10(I3,E10.2),/,10(I3,E10.2),/)
c      IF (IPRINT .GE. 3) WRITE (7,38) (K,C1(K),K=1,NN)
c      IF (IPRINT .GE. 3) WRITE (7,38) (K,C2(K),K=1,NN)
   50 IIK = IX
      IX=0
      DO 270 IL=1,N
      IY=3*LL(IL)
  ...
  270 IX=IX+IY
	...
	WDUM(LOC) = WAVE(BB)
	PDUM(LOC) = POWER(BB)
c	IF (LOC .EQ. 5) WRITE (7,345) N,(JDUM(K),LDUM(K),WDUM(K),PDUM(K),
c     1  K = 1,5)
  345   FORMAT (I4,5(I4,I3,F8.2,E10.3))
	LOC = MOD(LOC,5)
  360   CONTINUE
  370   IX = IX + IY
c	IF (LOC .NE. 0) WRITE(7,345) N,(JDUM(K),LDUM(K),WDUM(K),PDUM(K),
c     1  K=1,LOC)
  380 CONTINUE	
c      IF (IPRINT .NE. 0) WRITE(7,40) PCOOL,RE,TU
...
c      IF (LOC .EQ. 5) WRITE(7,1415) N,(JDUM(K),LDUM(K),WDUM(K),PDUM(K),
c     1  K = 1,5)
 ...
 1450 IX = IX + IY
      IL = LOC-1
c      IF (LOC .NE. 1) WRITE(7,1415) N,(JDUM(K),LDUM(K),WDUM(K),PDUM(K),
c     1  K = 1,IL)
...
c      WRITE(7,1460) N
 1460 FORMAT ('  ***** FORBIDDEN LINES ***** N=',I3)
      ...
      WDUM(LOC) = FWV(I)
      PDUM(LOC) = FTOT(I)*C
c      IF (LOC .EQ. 5) WRITE(7,212) N,(LDUM(K),WDUM(K),PDUM(K),K=1,5)
  212 FORMAT (1X,I2,5(I6,F9.1,E10.3))
      LOC = MOD(LOC,5) + 1
 1470 CONTINUE
      IL = LOC-1
c      IF (LOC .NE. 1) WRITE(7,212) N,(LDUM(K),WDUM(K),PDUM(K),K=1,IL)
 1500 CONTINUE
      RETURN
      END
