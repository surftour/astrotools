...
if do_allstars eq 1 then begin
    ...
    ;hsml= 10.0 * fload_allstars_hsml(1)
...
if fancy_center eq 1 then begin
	...
	if fload_npart(5) gt 1 then begin
		...
		center_bh[0]= (center1[0] + center2[0])/2.
		center_bh[1]= (center1[1] + center2[1])/2.
		center_bh[2]= (center1[2] + center2[2])/2.
	endif
...
fancy_center= 0
if fancy_center eq 1 then begin
	;mw_bhid= 817955     ; valid for the high resolution run (bhires)
	;m31_bhid= 2145299

	mw_bhid= 500001      ; v[4,5,6]
	;m31_bhid= 2145299
...
;snaparr= [0, 8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 100]
;snaparr= [0, 7, 21, 28, 35, 42, 49, 56, 63, 70, 77, 105]
snaparr= [35, 38, 42, 45, 49, 52, 56, 59, 63, 66, 70, 73, 77]
...
;xlenarr= [400.0, 400.0, 200.0, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0, 75.0]
;xlenarr= [400.0, 400.0, 200.0, 100.0, 100.0, 100.0, 100.0, 75.0, 75.0, 50.0, 50.0, 50.0]
xlenarr= [570.0, 570.0, 285.0, 140.0, 140.0, 140.0, 140.0, 105.0, 105.0, 60.0, 60.0, 60.0] * .7


timelbl= ['Today', '+.5 Gyr', '+1.0 Gyr', '+1.5 Gyr', '+2.0 Gyr', '+2.5 Gyr', $
	  '+3.0 Gyr', '+3.5 Gyr', '+4.0 Gyr', '+4.5 Gyr', '+5.0 Gyr', '+5.5 Gyr']
;timelbl(*)= ' '
;timelbl(*)= ' '



filename=frun+'/panelfig_starszoom.eps'
...
;--------------------------------------
	...
	;setup_plot_stuff, 'ps', filename=filename, colortable= 4
        ;setup_plot_stuff, 'ps', filename=filename, colortable= 4, newxsize=18, newysize=24
        setup_plot_stuff, 'ps', filename=filename, colortable= 3, newxsize=18, newysize=24
        ;setup_plot_stuff, 'ps', filename=filename, colortable= 1, newxsize=18, newysize=24
        ;setup_plot_stuff, 'ps', filename=filename, colortable= 0, newxsize=18, newysize=24
	...
	do_panel, frun, snaparr[0], xlenarr[0], x0, y3, xs, ys, $
		rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[0], /colorbar, /drawcenters
	do_panel, frun, snaparr[1], xlenarr[1], x1, y3, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[1], /drawcenters
	do_panel, frun, snaparr[2], xlenarr[2], x2, y3, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[2], /drawcenters
	do_panel, frun, snaparr[3], xlenarr[3], x0, y2, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[3], /drawcenters
	do_panel, frun, snaparr[4], xlenarr[4], x1, y2, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[4], /drawcenters
	do_panel, frun, snaparr[5], xlenarr[5], x2, y2, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[5], /drawcenters
	do_panel, frun, snaparr[6], xlenarr[6], x0, y1, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[6], /drawcenters
	do_panel, frun, snaparr[7], xlenarr[7], x1, y1, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[7], /drawcenters
	do_panel, frun, snaparr[8], xlenarr[8], x2, y1, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[8], /drawcenters
	do_panel, frun, snaparr[9], xlenarr[9], x0, y0, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[9]
	do_panel, frun, snaparr[10], xlenarr[10], x1, y0, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[10]
	;do_panel, frun, snaparr[1], xlenarr[0], x2, y0, xs, ys, $
	;	rotate_phi=rotate_phi, rotate_theta=rotate_theta, /draw_circle
	do_panel, frun, snaparr[11], xlenarr[11], x2, y0, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, tlbl=timelbl[11]
	...
	; -----------
		...
		;xyouts, 0.07, 0.83, msg, /normal, size= 1.7, charthick=3.0, color= 0   ; 0=black, 1=white
       	...
       	xyouts, x1+0.05, y3+0.05, '!6MW', /normal, size= 1.2, charthick=3.0, color= 0
       	xyouts, x1+0.24, y4-0.13, '!6M31', /normal, size= 1.2, charthick=3.0, color= 0
...
pro do_panel, frun, snapnum, xlen, $
	...
	tlbl=tlbl, drawcenters=drawcenters
	...
	if keyword_set(tlbl) then TimeLbl= tlbl
	...
	x1= x0 + xs
	y1= y0 + ys
	overplot_axes, x0, y0, x1, y1, TimeLbl, xlen, colorbar=colorbar    ;, /add_j_vector
	...
	;snaplbl= '('+strcompress(string(snapnum),/remove_all)+')'
		...
		if keyword_set(drawcenters) then begin
			...
			idx= where(cp_time le (fload_time(1)+0.05))
			...
			; center a little differently
			cidx= n_elements(idx)-1
			cx= (cp_cen1_x(idx) + cp_cen2_x(idx))/2.
			cy= (cp_cen1_y(idx) + cp_cen2_y(idx))/2.
			cp_cen1_x= cp_cen1_x - cx
			cp_cen1_y= cp_cen1_y - cy
			cp_cen2_x= cp_cen2_x - cx
			cp_cen2_y= cp_cen2_y - cy

			oplot, cp_cen1_x, cp_cen1_y, psym=-3, linestyle=2, color= 50, thick=1.0
			oplot, cp_cen2_x, cp_cen2_y, psym=-3, linestyle=1, color= 100, thick=1.0
...
pro overplot_axes, xx0, yy0, xx1, yy1, thetimelbl, xlen, $
	...
	if not keyword_set(colorbar) then begin
		;xlenlbl= strcompress(string(2.0*xlen),/remove_all)
		side= 2.0*xlen/0.7
		xlenlbl= strcompress(string(side),/remove_all)
		if (side) ge 1.0 then digs= 1
		if (side) ge 10.0 then digs= 2
		if (side) ge 100.0 then digs= 3
		if (side) ge 1000.0 then digs= 4
