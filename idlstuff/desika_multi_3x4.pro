...
;fancy_center2= 1
fancy_center2= 0
if fancy_center2 eq 1 then begin

	; do it by number of BHs
	if fload_npart(5) gt 1 then begin
		center= [0,0,0]
	endif else begin
		center= fload_center_alreadycomp(1)
...
pro produce_multiplot, junk, filename=filename, $
			evensampling=evensampling
...
if not keyword_set(junk) then begin
   print, "  "
   print, "produce_multiplot, junk, /evensampling"
...
frun= "data/z3/b5e"
xlenarr(*)= 35.0
snaparr= [15, 30, 38, 40, 41, 42, 43, 44, 45, 46, 50, 80]
filename=frun+'/panel_gas4.eps'
;---------------------------
...
if not keyword_set(frun) then begin
	frun=junk
	if not keyword_set(filename) then filename=frun+"/panelfig.eps"
endif
...
if keyword_set(evensampling) then begin
	...
	if not keyword_set(snaparr) then begin
	  print, "  "
	  print, "  PROBLEM: either set snaparr internally or use /evensampling"
	  print, "  "
	  return
	endif
endelse
...
;--------------------------------------
	...
	do_panel, frun, snaparr[2], xlenarr[2], x2, y3, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, /desika_box
	do_panel, frun, snaparr[3], xlenarr[3], x0, y2, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, /desika_box
	do_panel, frun, snaparr[4], xlenarr[4], x1, y2, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, /desika_box
	do_panel, frun, snaparr[5], xlenarr[5], x2, y2, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, /desika_box
	do_panel, frun, snaparr[6], xlenarr[6], x0, y1, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, /desika_box
	do_panel, frun, snaparr[7], xlenarr[7], x1, y1, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, /desika_box
	do_panel, frun, snaparr[8], xlenarr[8], x2, y1, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, /desika_box
	do_panel, frun, snaparr[9], xlenarr[9], x0, y0, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, /desika_box
	do_panel, frun, snaparr[10], xlenarr[10], x1, y0, xs, ys, rotate_phi=rotate_phi, rotate_theta=rotate_theta, /desika_box
	;do_panel, frun, snaparr[10], xlenarr[10], x2, y0, xs, ys, $
	do_panel, frun, snaparr[11], xlenarr[11], x2, y0, xs, ys, $
		rotate_phi=rotate_phi, rotate_theta=rotate_theta
		;rotate_phi=rotate_phi, rotate_theta=rotate_theta, /panel6_seen_edgeon
	...
	; -----------
		...
		;xyouts, 0.07, 0.83, msg, /normal, size= 1.2, charthick=3.0, color= 0   ; 0=black, 1=white
        	xyouts, x0+0.09, y4-0.03, '!6Gyr', /normal, size= 1.2, charthick=3.0, color= 0
        	;xyouts, x0+0.09, y4-0.03, '!6Gyr/h', /normal, size= 1.2, charthick=3.0, color= 0
        endif
	...
	xyouts, x1-0.10, y3+0.015, xlenlbl, /normal, size= 1.2, charthick=3.0, color= 0
...
pro do_panel, frun, snapnum, xlen, $
	...
	;snaplbl= '('+strcompress(string(snapnum),/remove_all)+')'
		...
		if keyword_set(desika_box) then begin
			...
			;if total(abs(center_used)) gt 0.0 then c= center_used else c= [0,0,0]
			c= [0,0,0]

			d= boxlen/2.
			oplot, [c(0)-d, c(0)+d], [c(1)-d, c(1)-d], color= 200, thick=5.0, psym=-3
			oplot, [c(0)-d, c(0)+d], [c(1)+d, c(1)+d], color= 200, thick=5.0, psym=-3
			oplot, [c(0)+d, c(0)+d], [c(1)+d, c(1)-d], color= 200, thick=5.0, psym=-3
			oplot, [c(0)-d, c(0)-d], [c(1)+d, c(1)-d], color= 200, thick=5.0, psym=-3
		endif
		...
		if keyword_set(showbhs) and fload_npart(5) ge 1 then begin
			...
			if n_bh gt 1 then begin
				...
				bh_center_1= bh_center_1 - center
				oplot, [bh_center_1[0]], [bh_center_1[1]], psym=7, thick=6.0, color=150, symsize=thissymsz
				...
				bh_center_2= bh_center_2 - center
				oplot, [bh_center_2[0]], [bh_center_2[1]], psym=7, thick=6.0, color=150, symsize=thissymsz
		...
		if overplot_ids eq 1 then begin
			fancy_center= 1
			if fancy_center eq 1 then center= fload_center_alreadycomp(1)
			oplot_contour_add_idlist, 'junk', center=center
...
pro overplot_axes, xx0, yy0, xx1, yy1, thetimelbl, xlen, $
	...
	if keyword_set(thetimelbl) then begin
