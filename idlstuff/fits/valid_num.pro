...
;
; CALLING SEQUENCE: 
;     IDL> status = valid_num(string  [,value]  [,/iinteger])
;    
...
FUNCTION valid_num, string, value, IINTEGER=iinteger

		;**** Set defaults for keyword ****
  IF NOT (KEYWORD_SET(iinteger)) THEN iinteger=0
  ...
  if iinteger eq 0 then stage = 1 else stage = 6
...
;
  ...
  if (n_params() eq 2) and ok then begin
	if keyword_set(iinteger) then value = long(string) else	$
		value = double(string)
  endif
