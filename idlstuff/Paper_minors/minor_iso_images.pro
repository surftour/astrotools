pro produce_one_image, frun, snapnum, xlen, $
			center=center, $
			colorbar=colorbar, $
			crude=crude, $
			do_allstars=do_allstars, $
			do_gas=do_gas, $
			filename=filename, $
			fitstoo=fitstoo, $
...
if keyword_set(do_gas) then begin
    npart= fload_npart(0)
    N= long(npart)
...
if keyword_set(do_allstars) then begin
...
;frun= "/raid4/tcox/isolated/G3_n0" & snapnum= 20 & msg='G3'
;frun= "/raid4/tcox/isolated/G2_n0" & snapnum= 10 & msg='G2'
frun= "/raid4/tcox/isolated/G1_n0" & snapnum= 20 & msg='G1'
;frun= "/raid4/tcox/isolated/G0_n0" & snapnum= 20 & msg='G0'
...
snaplbl_len=strlen(snaplbl)
snaplbl=strmid(snaplbl,snaplbl_len-3,3)
filename=frun+'/panel_'+snaplbl+'.eps'
...
;--------------------------------------
	...
	;setup_plot_stuff, 'ps', filename=filename, colortable= 4
        ;setup_plot_stuff, 'ps', filename=filename, colortable= 4, newxsize=6, newysize=18
        ;setup_plot_stuff, 'ps', filename=filename, colortable= 3, newxsize=6, newysize=18
        setup_plot_stuff, 'ps', filename=filename, colortable= 1, newxsize=6, newysize=18
        ;setup_plot_stuff, 'ps', filename=filename, colortable= 0, newxsize=6, newysize=18
	...
	; top: stars
	;do_panel, frun, -1, xlen, x0, y2, xs, ys, /colorbar, /do_allstars, /loadedsnap
	do_panel, frun, -1, xlen, x0, y2, xs, ys, /do_allstars, /loadedsnap


	; middle: gas
	;do_panel, frun, -1, xlen, x0, y1, xs, ys, /colorbar, /do_gas, /loadedsnap
	do_panel, frun, -1, xlen, x0, y1, xs, ys, /do_gas, /loadedsnap


	; bottom: half gas, half stars, from side view
	rotate_phi=0.0
	rotate_theta=0.0
	do_panel, frun, -1, xlen/2, x0, y0+ys/2, xs, ys/2, /do_allstars, /loadedsnap, /halfit, /xz

	do_panel, frun, -1, xlen/2, x0, y0, xs, ys/2, /do_gas, /loadedsnap, /halfit, /xz



	; place time in upper left
	; ---------------------------
        thetimelbl= '!6'+fload_timelbl(1.0,2,/noteq)+' Gyr'
        ;xyouts, x0+0.04, y3-0.03, thetimelbl, /normal, size= 1.2, charthick=3.0, color= 0
	...
	; -----------
		...
		xyouts, x0+0.07, y3-0.05, msg, /normal, size= 1.7, charthick=3.0, color= 0   ; 0=black, 1=white
        	;xyouts, x0+0.08, y2-0.03, '!6Gyr', /normal, size= 1.2, charthick=3.0, color= 0

		xyouts, x0+0.05, y2+0.02, 'stars', /normal, size= 1.3, charthick=3.0, color= 0   ; 0=black, 1=white
		xyouts, x0+0.05, y1+0.02, 'gas', /normal, size= 1.3, charthick=3.0, color= 0   ; 0=black, 1=white
		xyouts, x0+0.05, y0+ys/2+0.02, 'stars', /normal, size= 1.3, charthick=3.0, color= 0   ; 0=black, 1=white
		xyouts, x0+0.05, y0+0.02, 'gas', /normal, size= 1.3, charthick=3.0, color= 0   ; 0=black, 1=white
        endif
...
pro do_panel, frun, snapnum, xlen, $
	...
	do_gas=do_gas, do_allstars=do_allstars, loadedsnap=loadedsnap, $
	xz=xz, halfit=halfit
	...
	if keyword_set(halfit) then Pic= ReturnImage[*,121:360]


	if keyword_set(do_allstars) then fload_newcolortable, 4
	if keyword_set(do_gas) then fload_newcolortable, 1
		  ...
		  if keyword_set(do_gas) then begin
			...
			numxticks= 4
                  ...
                  axis, xaxis=1, xrange=[alog10(mi)+4.0,alog10(ma)+4.0], xstyle=1, color= 0, $
                        ...
                        xtitle= '!6Log !4R!D!6 !N(M!D!9n!6!N pc!E-2!N)'
