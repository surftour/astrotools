...
; -------------------------------------
	...
	x0= 0.02
	xs= 0.32   ; assumes 3 panels
	x1= x0+xs
	x2= x0+xs+xs
	x3= x0+xs+xs+xs

        y0= 0.02
	ys= 0.48   ; assumes 2 panels
        y1= y0+ys
	y2= y0+ys+ys
	...
	;  1
	; ---
        !p.position=[x0,y1,x1,y2] & !p.ticklen=0.03
        plot,[0],[0], psym=3, xrange=[xmin,xmax], yrange=[ymin,ymax], /noerase, color= 0, $
                      ...
                      xthick=4.0, ythick=4.0, xtickformat='(a1)', ytickformat='(a1)'

	xyouts, x0+0.02, y2-0.05, TimeLbl1, /normal, size= 1.2, charthick=3.0, color= 0


        ;  2
        ; ---
        !p.position=[x1,y1,x2,y2] & !p.ticklen=0.03
        plot,[0],[0], psym=3, xrange=[xmin,xmax], yrange=[ymin,ymax], /noerase, color= 0, $
                      xcharsize=0.01, ycharsize=0.01, xstyle=1, ystyle=1, /normal, /nodata, $
                      xthick=4.0, ythick=4.0, xtickformat='(a1)', ytickformat='(a1)'

        xyouts, x1+0.02, y2-0.05, TimeLbl2, /normal, size= 1.2, charthick=3.0, color= 0

        ;  3
        ; ---
        !p.position=[x2,y1,x3,y2] & !p.ticklen=0.03
        plot,[0],[0], psym=3, xrange=[xmin,xmax], yrange=[ymin,ymax], /noerase, color= 0, $
                      xcharsize=0.01, ycharsize=0.01, xstyle=1, ystyle=1, /normal, /nodata, $
                      xthick=4.0, ythick=4.0, xtickformat='(a1)', ytickformat='(a1)'

        xyouts, x2+0.02, y2-0.05, TimeLbl3, /normal, size= 1.2, charthick=3.0, color= 0
        ...
        xyouts, x0+0.02, y1-0.05, TimeLbl4, /normal, size= 1.2, charthick=3.0, color= 0

        ;  5
        ; ---
        !p.position=[x1,y0,x2,y1] & !p.ticklen=0.03
        plot,[0],[0], psym=3, xrange=[xmin,xmax], yrange=[ymin,ymax], /noerase, color= 0, $
                      xcharsize=0.01, ycharsize=0.01, xstyle=1, ystyle=1, /normal, /nodata, $
                      xthick=4.0, ythick=4.0, xtickformat='(a1)', ytickformat='(a1)'

        xyouts, x1+0.02, y1-0.05, TimeLbl5, /normal, size= 1.2, charthick=3.0, color= 0

        ;  6
        ; ---
        !p.position=[x2,y0,x3,y1] & !p.ticklen=0.03
        plot,[0],[0], psym=3, xrange=[xmin,xmax], yrange=[ymin,ymax], /noerase, color= 0, $
                      xcharsize=0.01, ycharsize=0.01, xstyle=1, ystyle=1, /normal, /nodata, $
                      xthick=4.0, ythick=4.0, xtickformat='(a1)', ytickformat='(a1)'

        xyouts, x2+0.02, y1-0.05, TimeLbl6, /normal, size= 1.2, charthick=3.0, color= 0

	if panel6_seen_edgeon eq 1 then begin
		xyouts, x2+0.02, y1-0.10, "side view", /normal, size= 1.2, charthick=3.0, color= 0
	endif
		...
		;
                ...
                if keyword_set(colorbar2) then begin
                  ...
                  axis, xaxis=1, xrange=[alog10(mi)+4.0,alog10(ma)+4.0], xstyle=1, color= 0, $
                        xcharsize=1.20, charthick=3.0, xthick= 3.0, /normal, $
                        xticks=3, xtickformat='(a1)', xticklen=0.05, $
			...
			; +4.0 above comes from converting to these units
                  ...
                  axis, xaxis=0, xrange=[alog10(mi)+4.0,alog10(ma)+4.0], xstyle=1, $
                        xticks=3,xcharsize=1.20, ycharsize=1.00, charthick=3.0, xthick= 3.0, $
