...
;        MAIN PROGRAM
;----------------------------------------------------------------------
PRO tj_kinemetry, xbin, ybin, moment, rad, pa, q, cf, $
	NTRM=NTRM, ERROR=error, SCALE=scale, NAME=name, $
	PAQ=paq, NPA=npa, NQ=nq, RANGEQ=rangeq, ALL=all, EVEN=even,$
	VSYS=VSYS, VELCIRC=velCirc, VELKIN=velkin, ER_CF=er_cf, $
	ER_PA=er_pa, ER_q=er_q, XELLIP=xellip, YELLIP=yellip, $
...
FOR i=0, nrad-1 DO BEGIN
	...
	fname="tj_test_"+strcompress(string(i),/remove_all)+".eps"
	...
	loadct, 25, /silent
		...
		CONTOUR, moment>mn<mx, xbin, ybin, /FILL, /ISO, LEVELS=levels, $
		    /XSTYLE, /YSTYLE, /IRREGULAR, XTITLE='kpc/h', YTITLE='kpc/h'
        ENDIF ELSE BEGIN
	...
	ENDELSE
        ...
        LOADCT, 4, /silent
        CONTOUR, ALOG(chi2_grid), pa_grid, q_grid, NLEVELS=15, XTITLE='PA (deg)', YTITLE='q', /FILL, charsize= 1.5
	...
	IF KEYWORD_SET(EVEN) THEN BEGIN
	  ...
	  ELSE $
          ...
          PLOT, theta*!RADEG, SIGRANGE(momEll-coeff[0]), PSYM=-4, XTITLE='!7h!X!N', YTITLE='residuals', YRANGE=[-20,20], charsize= 1.5
	...
	ENDIF ELSE BEGIN
	  ...
	  IF N_ELEMENTS(error) gt 1 THEN $
	        PLOTERROR, theta[w]*!RADEG, momEll[w], er_momEll, PSYM=-1, TITLE=rad[i]*scale, XTICKFORMAT='(A1)', YTITLE='V', charsize= 1.5 $
	  ELSE $
        	PLOT, theta[w]*!RADEG, momEll[w], PSYM=-1, TITLE=rad[i], XTICKFORMAT='(A1)', YTITLE='V', charsize= 1.5
          OPLOT, theta*!RADEG, coeff[0]+b1*COS(theta), PSYM=-4, COLOR=200

          PLOT, theta*!RADEG, SIGRANGE(momEll-coeff[0]-b1*COS(theta)), PSYM=-4, XTITLE='!7h!X!N', YTITLE='residuals', YRANGE=[-20,20], charsize= 1.5
