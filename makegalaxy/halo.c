...
void compute_velocity_dispersions_halo(void)
{
  ...
  for(i = 0; i <= RSIZE; i++)
      ...
      for(j = ZSIZE - 1, VelDispRz_halo[i][ZSIZE] = 0; j >= 0; j--)
	{
	  ...
	  if(fabs(yl[j + 2]) > 1e-100 && fabs(yl[j + 1]) > 1e-100)
	    VelDispRz_halo[i][j] += qromb(splint_xl_yl_D2yl, list_z[j], list_z[j + 1]);
	}
    }
  ...
  for(i = 0; i <= RSIZE; i++)
      ...
      for(j = ZSIZE - 1, VelDispRz_dR_halo[i][ZSIZE] = 0; j >= 0; j--)
	{
	  ...
	  if(fabs(yl[j + 2]) > 1e-100 && fabs(yl[j + 1]) > 1e-100)
	    VelDispRz_dR_halo[i][j] += qromb(splint_xl_yl_D2yl, list_z[j], list_z[j + 1]);
	}
	  ...
	  if(rho > 0)
	    {
	      if(i > 0)
		VelDispPhi_halo[i][j] =
		  R / rho * (VelDispRz_dR_halo[i][j] - VelDispRz_halo[i][j]) / (list_RplusdR[i] - list_R[i]);
	      else
...
if(Dphi_R[i][j] < 0.0)
{
printf("i= %d  j= %d  z=%g  R=%g  Dphi_R= %g   Dphi_z= %g\n",i, j, list_z[j], list_R[i], Dphi_R[i][j], Dphi_z[i][j]); fflush(stdout);
printf("R= %g   VelVc2_halo[%d][%d]=  %g\n",R,i,j,VelVc2_halo[i][j]); fflush(stdout);
}

	  VelDispPhi_halo[i][j] += VelVc2_halo[i][j] + VelDispRz_halo[i][j];

	  /* VelStreamPhi_halo[i][j] = halo_spinfactor * sqrt(VelVc2_halo[i][j]); */
	  VelStreamPhi_halo[i][j] = halo_spinfactor * sqrt(fabs(VelVc2_halo[i][j]));
/*
printf("VelStreamPhi_halo[%d][%d]=  %g\n",i,j,VelStreamPhi_halo[i][j]); fflush(stdout);
*/
