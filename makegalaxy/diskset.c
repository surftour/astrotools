...
double set_disk_velocities(void)
{
  ...
  for(i = 1; i <= N_DISK; i++)
	...
	/* ir = (log10(R) - log10(Baselen)) / (log10(LL) - log10(Baselen)) * (RSIZE - 1) + 1; */
	...
	/* iz = (log10(z) - log10(Baselen)) / (log10(LL) - log10(Baselen)) * (ZSIZE - 1) + 1; */
...
double set_disk_positions(void)
{
  ...
  for(i = 1; i <= N_DISK; i++)
    /* old method where M_DISK include gas
