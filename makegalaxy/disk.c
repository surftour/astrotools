...
/*
      ...
      nint = force_treeevaluate(pos, 0.1 * H, acc);      - no problems with dphi_R
*/
      nint = force_treeevaluate(pos, GravSoftening, acc);
  ...
  if(M_DISK > 0)
    {
      ...
      if(R>LL)
	pot = M_DISK / sqrt(R*R + z*z);
      else
	pot = potential_treeevaluate(pos, GravSoftening);
...
double comp_rho_disk(double R, double z)
{
  ...
  /* M_DISK now includes only the stellar disk
  x =
