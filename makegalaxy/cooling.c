...
double DoCooling(double u_old, double rho, double dt, double *ne_guess)
{
  ...
  /* note that there is no correction for the h^-1 factor
     since we've now taken this out of the entire code */
  rho *= UnitDensity_in_cgs;               	/* convert to physical cgs units */
...
#ifdef  MOLECULAR_COOLING 
/** 	following Sanchez-Salcedo 2002 ApJ 577, 768  **/


      	if (T <= 1e4 && T > 6102.0) 
  	  LambdaMol = 2.0e8*pow(T, 3.67)*nH0*nH0*PROTONMASS*PROTONMASS;
	else if	(T <= 6102.0 && T > 313.0)
	  LambdaMol = 1.11e20*pow(T, 0.56)*nH0*nH0*PROTONMASS*PROTONMASS;
	else if (T <= 313.0 && T  > 141.0)
	  LambdaMol = 9.1e18*pow(T,1.0)*nH0*nH0*PROTONMASS*PROTONMASS;
	else if (T <= 141.0 && T > 10.0 ) 
	  LambdaMol =  3.42e16*pow(T,2.12)*nH0*nH0*PROTONMASS*PROTONMASS;
	else
	  LambdaMol = 0.0;

 	  Lambda = LambdaExc + LambdaIon + LambdaRec + LambdaFF + LambdaMol;
#else
