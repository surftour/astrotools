...
void structure(void)
{
  ...
  N_TOTAL= N_HALO + N_GAS + N_DISK + N_BULGE + N_BLACKHOLE;

  M_GAS = M_DISK * GasFraction;
  M_DISK = M_DISK - M_GAS;        /* New addition to separate out the stellar and gas disks */
  M_HALO = M200 - M_GAS - M_DISK - M_BULGE - M_BLACKHOLE;

  RH = RS * sqrt(2 * (log(1 + CC) - CC / (1 + CC)));	/* scale length of Hernquist profile */


  /* change to a version corrected for gas content 
  jhalo = LAMBDA * sqrt(G) * pow(M200, 1.5) * sqrt(2 * R200 / fc(CC)); */
  jhalo = (1.0 - GasFraction) * LAMBDA * sqrt(G) * pow(M200, 1.5) * sqrt(2 * R200 / fc(CC));
  jdisk = JD * jhalo;
  ...
  /* if parameter file set H > 0, then don't calculate scale
     length from the halo angular momentum */
  if(H <= 0.0)
    {
