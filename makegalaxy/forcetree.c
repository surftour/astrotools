...
double potential_treeevaluate(double *pos, double softening)
{
  struct NODE *nop = 0;
  int no, ninteractions;
  double r2, dx, dy, dz, mass, r, fac, u, h, h_inv, h3_inv;
  double pot=0, pos_x, pos_y, pos_z;
...
*/
      ...
      else
	{
	  u = r * h_inv;
	  if(u < 0.5)
	    fac = mass * h_inv * (10.666666666667 + u * u * (32.0 * u - 38.4));
	  else
	    fac =
	      mass * h_inv * (21.333333333333 - 48.0 * u +
			       38.4 * u * u - 10.666666666667 * u * u * u - 0.066666666667 / (u * u * u));
	}
...
void force_treeallocate(int maxnodes, int maxpart)	/* usually maxnodes=0.7*maxpart is sufficient */
{
  ...
  if(!(Nodes_base = malloc(bytes = (MaxNodes + 1) * sizeof(struct NODE))))
    {
      printf("failed to allocate memory for %d tree-nodes (%zu bytes).\n", MaxNodes, bytes);
      exit(3);
    }
  allbytes += bytes;
  ...
  if(!(Nextnode = malloc(bytes = (maxpart) * sizeof(int))))
    {
      printf("Failed to allocate %d spaces for 'Nextnode' array (%zu bytes)\n", maxpart, bytes);
      exit(0);
    }
  allbytes += bytes;
