...
#define RELCHANGE_LIMIT 0.025
  ...
  DeltaM = mymalloc(N_gas * sizeof(double));
  ...
  if(ThisTask == 0)
    {
      printf("\nMESHRELAX: disp_rms=%g  press_rms=%g  actual_disp_rms=%g  max_disp=%g  minmass_all=%g maxmass_all=%g\n",
	     disp_rms, press_rms, disp_rms * fac, sqrt(max_disp2_all)*fac, 
             minmass_all, maxmass_all);
    }
...
void voronoi_meshrelax_calculate_gradients(void)
{
  ...
  Grad = mymalloc(N_gas * sizeof(struct grad_data));

  minmax = mymalloc(N_gas * sizeof(struct max_data));
