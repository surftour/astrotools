...
void compare_partitions(void)
{
  ...
  PP = (struct combined_data *) mymalloc(NumPart * sizeof(struct combined_data));
  ...
  numpart_list = (int *) mymalloc(sizeof(int) * World_NTask);
  ...
  if(ThisTask == 0 && World_ThisTask == 0)
      ...
      if(numPart1 != numPart2)
	{
      ...
      PP1 = (struct combined_data *) mymalloc(NMAX_CHECK * sizeof(struct combined_data));
      PP2 = (struct combined_data *) mymalloc(NMAX_CHECK * sizeof(struct combined_data));
