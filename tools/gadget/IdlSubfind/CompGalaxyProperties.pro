...
gal_list = where(SubhaloMassPerType(2,*) gt 0)  ; select subhalos with stars"

for gal=0, n_elements(gal_list)-1 do begin

   nr = gal_list(gal)

   ; get particles belonging to group
   
   pp = pos(*, SubhaloOffset(nr):SubhaloOffset(nr)+SubhaloLen(nr)-1)
   vv = vel(*, SubhaloOffset(nr):SubhaloOffset(nr)+SubhaloLen(nr)-1)
   tt = type(SubhaloOffset(nr):SubhaloOffset(nr)+SubhaloLen(nr)-1)

   ; now, let's only select the stars
   ...
   ; let's calculate the half-mass radius and the velocity dispersion

   r = sqrt((pp(0,*) - SubhaloPos(0, nr))^2 + (pp(1,*) - SubhaloPos(1, nr))^2 +  (pp(2,*) - SubhaloPos(2, nr))^2)
   
   ; sort r
   r = r(sort(r))

   rhalf = r(n_elements(r)/2)

   vmx = total(vv(0,*),/double) / n_elements(ind)
   vmy = total(vv(1,*),/double) / n_elements(ind)
   vmz = total(vv(2,*),/double) / n_elements(ind)

   disp = sqrt(total( (vv(0,*)-vmx)^2 + (vv(1,*)-vmy)^2 + (vv(2,*)-vmz)^2, /double)/ n_elements(ind))

   ; convert the measurements to physical
   ; radii and physical velocities
 
   rhalf *= Scalefac
   disp  *= sqrt(Scalefac)

   print, gal, "  Nstars=", n_elements(ind), " Rhalf=", rhalf, " Vel Disp=", disp
endfor
