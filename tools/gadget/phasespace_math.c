...
{
  fprintf(stderr, "Phase-space run-time error:\n");
  fprintf(stderr, "%s\n", error_text);
  fprintf(stderr, "EXIT\n");
  fflush(stderr);
  endrun(101278);
...
MyDouble *vector(long nl, long nh)
{
  ...
  v = (MyDouble *) malloc((size_t) ((nh - nl + 1 + NR_END) * sizeof(MyDouble)));
  if(!v)
...
MyDouble **matrix(long nrl, long nrh, long ncl, long nch)
{
  ...
  m = (MyDouble **) malloc((size_t) ((nrow + NR_END) * sizeof(MyDouble *)));
  if(!m)
  ...
  m[nrl] = (MyDouble *) malloc((size_t) ((nrow * ncol + NR_END) * sizeof(MyDouble)));
  if(!m[nrl])
