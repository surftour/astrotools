...
void cosmic_ray_diffusion(void)
{
  ...
  for(CRpop = 0; CRpop < NUMCRPOP; CRpop++)
    {
      CR_E0[CRpop] = (double *) mymalloc(N_gas * sizeof(double));
      CR_E0_Old[CRpop] = (double *) mymalloc(N_gas * sizeof(double));
      CR_E0_Kappa[CRpop] = (double *) mymalloc(N_gas * sizeof(double));
    }

  CR_E0_Residual = (double *) mymalloc(N_gas * sizeof(double));
  CR_E0_DVec = (double *) mymalloc(N_gas * sizeof(double));
  CR_E0_QVec = (double *) mymalloc(N_gas * sizeof(double));

  for(CRpop = 0; CRpop < NUMCRPOP; CRpop++)
    {
      CR_n0[CRpop] = (double *) mymalloc(N_gas *  sizeof(double));
      CR_n0_Old[CRpop] = (double *) mymalloc(N_gas * sizeof(double));
      CR_n0_Kappa[CRpop] = (double *) mymalloc(N_gas * sizeof(double));
    }

  CR_n0_Residual = (double *) mymalloc(N_gas * sizeof(double));
  CR_n0_DVec = (double *) mymalloc(N_gas * sizeof(double));
  CR_n0_QVec = (double *) mymalloc(N_gas * sizeof(double));
  ...
  for(i = 0; i < N_gas; i++)
      ...
      if(P[i].Type == 0)
	{
	  for(CRpop = 0; CRpop < NUMCRPOP; CRpop++)
		...
		kappa *= pow(SphP[i].d.Density * a3inv / All.CR_DiffusionDensZero, All.CR_DiffusionDensScaling);
  ...
  for(CRpop = 0; CRpop < NUMCRPOP; CRpop++)
    {
      cosmic_ray_diffusion_matrix_multiply(CR_E0_Old[CRpop], CR_E0_Residual, CR_n0_Old[CRpop], CR_n0_Residual, CRpop);
      ...
      for(i = 0; i < N_gas; i++)
	{
      ...
      while(iter < MAX_CR_DIFFUSION_ITER && glob_max_rel_change > CR_DIFFUSION_ITER_ACCURACY && CR_E0_delta1 > 0)
	{
					  ...
					  double *cr_n0_out, int CRpop)
{
  ...
  cr_E0_sum = (double *) mymalloc(N_gas * sizeof(double));
  cr_n0_sum = (double *) mymalloc(N_gas * sizeof(double));
	  ...
	  for(n = 0; n < numngb; n++)
	      ...
	      if(r2 < h_i2 || r2 < h_j * h_j)
		{
		  ...
		  if(r > 0)
			...
			kappa_mean = 2 * (CR_E0_Kappa_i * CR_E0_Kappa[CRpop][j]) / (CR_E0_Kappa_i + CR_E0_Kappa[CRpop][j]);
		      else
			kappa_mean = 0;
			...
			kappa_mean = 2 * (CR_n0_Kappa_i * CR_n0_Kappa[CRpop][j]) / (CR_n0_Kappa_i + CR_n0_Kappa[CRpop][j]);
		      else
			kappa_mean = 0;
