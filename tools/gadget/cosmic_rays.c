...
void CR_Tab_Initialize(void)
{
  ...
  for(i = 0; i <= TABLEN; i++)
	...
	{      	  
	  ...
	  Tab_beta[i][CRpop] = Beta((All.CR_Alpha[CRpop] - 2.0) * 0.5, (3.0 - All.CR_Alpha[CRpop]) * 0.5, 1.0 / (1.0 + q * q));
	  
	  for(j = 0; j <= ALPHAS; j++)
	    { 	      
	      Tab_injection_point[i][j][CRpop] = log(exp(Tab_tau_Cooling[i][CRpop]) * Tab_energy_fraction[i][j]);
...
double CR_Find_Qinj(double rho, double TimeScale, double alpha, double qstart, int CRpop)
{
  ...
  x = (TimeScale - Tab_injection_point[i1][j][CRpop]) / (Tab_injection_point[i2][j][CRpop] - Tab_injection_point[i1][j][CRpop]);
...
void CR_Particle_UpdateNonAdiabatics(SphParticle * Particle, double rQphys, int CRpop)
{
  ...
  if((fabs(Particle->CR_DeltaE[CRpop]) < CR_CHANGE_FAC * Particle->CR_E0[CRpop]) &&
  ...
  else
    {
       ...
       */
      if((Particle->CR_E0[CRpop] + Particle->CR_DeltaE[CRpop]) <= 0.0 || (Particle->CR_n0[CRpop] + Particle->CR_DeltaN[CRpop]) <= 0.0)
	{
	  /*
      ...
      Particle->CR_C0[CRpop] = Particle->CR_n0[CRpop] * (All.CR_Alpha[CRpop] - 1.0) * pow(Particle->CR_q0[CRpop], All.CR_Alpha[CRpop] - 1.0);
    }
...
void CR_Particle_Update(SphParticle * Particle, int CRpop)
{
  ...
  Particle->CR_n0[CRpop] = (Particle->CR_C0[CRpop] * pow(Particle->CR_q0[CRpop], 1.0 - All.CR_Alpha[CRpop]) / (All.CR_Alpha[CRpop] - 1));
...
double CR_Particle_GetDissipationTimescale(double rQ, double rDensity, int CRpop)
{
  ...
  egy = CR_mean_kinetic_energy(rQ, All.CR_Alpha[CRpop]) * pow(rQ, 1 - All.CR_Alpha[CRpop]) / (All.CR_Alpha[CRpop] - 1) / m_p;
...
double CR_Particle_GetThermalizationTimescale(double rQ, double rDensity, int CRpop)
{
  ...
  prefac =
    (m_e * LightSpeed * LightSpeed * LightSpeed * m_p) / (2 * M_PI * (All.CR_Alpha[CRpop] - 1.0) * rNe * square(e2));
...
double CR_corresponding_q(double qinj, double alpha, int CRpop)
{
  ...
  while(i2 - i1 > 1)
	...
	CR_kinetic_energy(qm) * (1 +
				 CR_Tab_GetThermalizationTimescale(qm,
								   1.0, CRpop) / CR_Tab_GetDissipationTimescale(qm,
													 1.0, CRpop));
      if(egym < egy)
	i1 = im;
      else
  ...
  egy1 =
    CR_kinetic_energy(q1) * (1 +
			     CR_Tab_GetThermalizationTimescale(q1, 1.0, CRpop) / CR_Tab_GetDissipationTimescale(q1,
													 1.0, CRpop));
  egy2 =
    CR_kinetic_energy(q2) * (1 +
			     CR_Tab_GetThermalizationTimescale(q2, 1.0, CRpop) / CR_Tab_GetDissipationTimescale(q2,
													 1.0, CRpop));
