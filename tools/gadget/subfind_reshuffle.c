...
void read_hsml_table(void)
{
  ...
  for(i = 0, Nfiles = 1; i < Nfiles; i++)
      ...
      if(!(fd = fopen(fname, "r")))
	{
      ...
      if(i == 0)
	NumPartPerFile = mymalloc((Nfiles + 1) * sizeof(long long));
...
void subfind_reshuffle_free(void)
{
  if(table_read_voronoi)
    {
      myfree_msg(NumPartPerFileVoronoi, "NumPartPerFileVoronoi in subfind_reshuffle");
      table_read_voronoi = 0;      
    }

  if(table_read)
    {
      myfree_msg(NumPartPerFile, "NumPartPerFile in subfind_reshuffle");
      table_read = 0;      
    }
}
...
void read_hsml_files(float *Values, int count, enum iofields blocknr, long long nskip)
{
  ...
  if(blocknr == IO_DMHSML_V || blocknr == IO_DMDENSITY_V)
      ...
      while(count > 0)
	{
	  ...
	  tmp = mymalloc(n_to_read * sizeof(float));
...
void read_hsml_table_voronoi(void)
{
  ...
  for(i = 0, NfilesVoronoi = 1; i < NfilesVoronoi; i++)
    {
      sprintf(fname, "%s/hsmldir_%03d/%s_%03d.%d", All.OutputDir, RestartSnapNum, "voronoi_hsml", RestartSnapNum, i);
      if(!(fd = fopen(fname, "r")))
	{
	  printf("can't open file `%s`\n", fname);
	  endrun(113839);
      ...
      if(i == 0)
	NumPartPerFileVoronoi = mymalloc((NfilesVoronoi + 1) * sizeof(long long));
...
void read_hsml_files_voronoi(float *Values, int count, enum iofields blocknr, long long nskip)
{
  ...
  while(count > 0)
	...
	read_hsml_table_voronoi();
      ...
      sprintf(fname, "%s/hsmldir_%03d/%s_%03d.%d", All.OutputDir, RestartSnapNum, "voronoi_hsml", RestartSnapNum,
	      filenr);
      if(!(fd = fopen(fname, "r")))
	{
	  printf("can't open file `%s`\n", fname);
