...
void cs_enrichment(void)
{
  ...
  MetalDataTmp = (struct metaldata_tmp *) mymalloc(N_gas * sizeof(struct metaldata_tmp));
  ...
  do
      ...
      for(j = 0, nimport = 0, Recv_offset[0] = 0, Send_offset[0] = 0; j < NTask; j++)
	{
      ...
      MetalDataGet = (struct metaldata_in *) mymalloc(nimport * sizeof(struct metaldata_in));
...
#define NUM_DENSITY_TAIL (2*All.DesNumNgb)
  ...
  rho = (float *) mymalloc(N_gas * sizeof(float));
  contrib_list = (int *) mymalloc(NTask * sizeof(int));
  contrib_offset = (int *) mymalloc(NTask * sizeof(int));
  ...
  rho_common = (float *) mymalloc(listlen * sizeof(float));
