...
void eddington(void)
{
  ...
  do
      ...
      for(j = 0; j < nexport; j++)
	{
	  place = DataIndexTable[j].Index;
#ifdef RT_VAR_DT
	  if(EddingtonDataOut[j].rt_flag)
	    SphP[place].rt_flag = EddingtonDataOut[j].rt_flag;
#endif
	  for(k = 0; k < 6; k++)
  ...
  for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
	...
	if(trace)
	...
	else
	  {
	    for(k = 0; k < 6; k++)
	      SphP[i].ET[k] = 0.0;
	    SphP[i].ET[0] = SphP[i].ET[1] = SphP[i].ET[2] = 1.0 / 3;
	  }
      }
...
int eddington_treeevaluate(int target, int mode, int *nexport, int *nsend_local)
{
  ...
  double r4, r2, r4_invfac, dx, dy, dz, stellar_mass, h;
  double pos_x, pos_y, pos_z;
  MyLongDouble ET[6] = { 0, 0, 0, 0, 0, 0 };
#ifdef RT_VAR_DT
  double c_light = C / All.UnitVelocity_in_cm_per_s;
  int rt_flag = 0;
#endif
  ...
  if(mode == 0)
    ...
    {
#ifdef RT_VAR_DT
      EddingtonDataResult[target].rt_flag = rt_flag;
#endif
      for(k = 0; k < 6; k++)
